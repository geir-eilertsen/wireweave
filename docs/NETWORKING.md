...
One honest caveat about trusting: CrowdSec re-reads its allowlist only when it restarts. So trusting an address lifts its block immediately and is remembered for good, but the permanent allowlist entry itself takes effect at CrowdSec's next restart. Vaier deliberately won't restart the engine for you — bouncing the thing guarding the door is how an operator ends up locked out. And if one ever does lock you out, `docker exec crowdsec cscli decisions delete --all` clears every active block from the host's shell.
