...
One tree spanning the whole fleet, at `/explorer.html` — the shape Vaier's UI is becoming. The tree is an address space, not a column: every entry has its own link, and the rail that draws it is optional. This covers the tree itself, the web terminal, the credential vault behind it, and the smaller pieces of UI polish that live alongside it.
...
The **path** you're standing on is the **address bar**, the **Inspector** on the right renders whatever you select (a machine shows its details, a directory shows its listing, a container shows what Vaier knows about it, a published service shows its route), and **⌘K** searches every entry in the fleet **by name** — including containers and published services, since they're entries in the same namespace now.
...
The same entry answers the other half of "what is authenticated where", in a second section beneath the credentials — with one difference that shapes everything about it: **Vaier does not hold this secret and never will.** A fleet credential is bytes Vaier keeps and copies. A **Claude sign-in** is a credential that Anthropic mints and the unmodified `claude` CLI on the machine keeps; Vaier orchestrates the sign-in and relays two things, and that is all. So there is no coverage strip here, no distribute, and nothing to heal.

One row per machine says where it stands — **Signed in** (and as which account), **Signed out**, **Claude isn't installed here**, **Unreachable**, **No shell access**, or **Couldn't tell**. That last one is not a polite way of saying signed out: Vaier goes to some trouble never to report a machine as signed out when it merely could not read the answer, because that is what sends you off to redo a sign-in nothing had undone. Machines with nowhere to sign in get no button rather than a button that fails.

Each row also names **the user Vaier acts as** on that machine, and that is not decoration. A Claude sign-in lives in one user's home directory, so it is a fact about a *user on* a machine and never about the machine — a box can report a perfectly healthy account for the login Vaier connects as while the automation actually doing the work there, running as another user, is signed out and expired. Both readings are true; a row that named only the machine would be the lie. Signing in here signs in the login Vaier connects as, and says nothing about any other login on that machine.

**Sign in** runs the CLI's own login on that machine and hands you back the link it printed. You open it in your own browser — approval happens on Anthropic's pages, never inside Vaier — and paste the code they show you into the box below the link; Vaier writes it into the CLI waiting on that machine and then asks the machine whether it worked. Neither the link nor the code is stored or logged, and the token that comes out of it never reaches Vaier at all. One sign-in at a time, and leaving the Credentials entry ends the one in progress rather than leaving a CLI waiting at a prompt on a machine nobody is going back to.

**Sign out** runs the CLI's own logout, so the CLI releases its own credential — Vaier never deletes or reads a credential file. And when your machines are signed in as more than one account, a quiet line says so: half a fleet on the wrong account looks perfectly healthy right up until something fails for a reason that makes no sense.
