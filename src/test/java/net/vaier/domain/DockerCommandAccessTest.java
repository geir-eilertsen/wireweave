...
/**
 * Whether Vaier's SSH user can drive Docker on a machine — learned on the disk sweep's own trip, because
...
class DockerCommandAccessTest {
    ...
    @Test
    void aProbeThatExitedCleanly_meansVaierCanRunDockerThere() {
        assertThat(DockerCommandAccess.readFrom(swept("VAIER-DOCKER-RC=0\nFilesystem 1024-blocks Used Available Capacity Mounted on")))
            .isEqualTo(DockerCommandAccess.GRANTED);
    }

    @Test
    void aProbeThatFailed_meansVaierCannotRunDockerThere() {
        // Exit 1 is what a docker.sock permission denial produces — but any non-zero exit means the same
        // thing for the question being asked: Vaier cannot drive Docker as this user on this machine.
    ...
    void aSweepThatLearnedNothing_recordsNothing_soAKnownFactIsNeverErasedByASilentTrip() {
        ...
        // about Docker. Writing UNKNOWN would forget a fact Vaier holds and re-offer a doomed button.
    ...
    void aSweepIsStillTakenOnAMachineAlreadyKnownToRefuse_soItSelfHealsWhenTheGroupIsFixed() {
        ...
        // operator adds Vaier's user to the docker group the next sweep records GRANTED.
