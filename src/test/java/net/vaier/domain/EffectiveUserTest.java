...
class EffectiveUserTest {
    ...
    void of_anyOtherLoginName_isNotPrivileged() {
        // A non-root user with passwordless sudo is still unprivileged for Vaier's own file operations —
        // the SFTP reads, writes and deletes the Explorer offers do not go through sudo. That is the
        // truthful answer to "what is the blast radius here", so it is the one Vaier gives.
    ...
    void of_isNotFooledByCaseOrPadding() {
        // SSH login names are case-sensitive on Linux, so "Root" is a different account and Vaier must not
