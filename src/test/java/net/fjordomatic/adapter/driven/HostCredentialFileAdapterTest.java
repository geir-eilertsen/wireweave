...
class HostCredentialFileAdapterTest {
    ...
    void save_managedKeypair_encryptsThePrivateKey_andRemembersItIsManaged() throws Exception {
        // #309: the private half of a Fjord-generated keypair never leaves the server, and it does not sit
    ...
    @Test
    void storesCredentialForTheFjordServerMachine_byItsCanonicalName() {
        // #311: the vault is name-keyed, so the Fjord-server singleton stores like any other machine.
        HostCredential credential = new HostCredential(mid(net.fjordomatic.domain.LanAnchor.FJORD_SERVER_NAME),
            "root", AuthMethod.PASSWORD, "host-pw", null, false);

        adapter.save(credential);

        assertThat(adapter.getByMachine(mid(net.fjordomatic.domain.LanAnchor.FJORD_SERVER_NAME))).contains(credential);
    }
