...
class AdminNotificationEmailAdapterTest {
    ...
    private FjordConfig smtpConfigured() {
    ...
    void sendToAdmins_skipsWhenSmtpHostNotConfigured() {
        when(configPersistence.load()).thenReturn(Optional.of(FjordConfig.builder().domain("example.com").build()));
    ...
    void notifyNewPendingIdentity_sendsAccessRequestEmailToEveryAdmin() {
        ...
        assertThat(recipients.getValue()).containsExactly("alice@example.com");
        assertThat(subject.getValue()).isEqualTo("[Fjord-O-Matic] New access request awaiting approval");
    ...
    void notifyNewPendingIdentity_skipsWhenSmtpNotConfigured() {
        when(configPersistence.load()).thenReturn(Optional.of(FjordConfig.builder().build()));
