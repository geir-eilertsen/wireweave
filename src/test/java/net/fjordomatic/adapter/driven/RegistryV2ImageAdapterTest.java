...
import net.fjordomatic.domain.ImageReference;
...
class RegistryV2ImageAdapterTest {
    ...
    void resolvesADigestOverTheAnonymousBearerTokenFlow() throws Exception {
        ...
        // The token is requested for exactly this repository, pull-only: Fjord never asks for write scope.
    ...
    void servesARepeatLookupFromCacheWithoutTouchingTheRegistryAgain() throws Exception {
        ...
        // optimisation here, it is what keeps Fjord under the limit.
        respondInOrder(response(200, Map.of("Docker-Content-Digest", List.of("sha256:served")), ""));
    ...
    void doesNotCacheAFailureSoATransientOutageIsRetriedOnTheNextSweep() throws Exception {
        // Caching "unknown" for 24h would mean one blip blinds Fjord for a day.
        when(httpClient.send(any(HttpRequest.class), any())).thenAnswer(invocation -> {
    ...
    void aForcedLookup_returnsWhatTheRegistryServesNow_notWhatItServedThisMorning() throws Exception {
        ...
        // the image they just updated — Fjord looking broken at the exact moment it is being checked.
        respondInOrder(
