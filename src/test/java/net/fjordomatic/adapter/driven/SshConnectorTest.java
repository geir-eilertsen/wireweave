...
class SshConnectorTest {
    ...
    // meant to sit idle for arbitrarily long (that's the whole point of the persistent shell), so Fjord's
    // own client must never be the one to end it.
    @Test
    void establishedClient_disablesIdleTimeout_soAnIdleShellIsNeverKilledByFjordItself() throws Exception {
