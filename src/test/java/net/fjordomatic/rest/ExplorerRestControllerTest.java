...
import net.fjordomatic.application.BrowseFilesUseCase;
import net.fjordomatic.application.BrowseFilesUseCase.MachineDirectory;
import net.fjordomatic.application.DeleteFileUseCase;
import net.fjordomatic.application.DownloadFileUseCase;
import net.fjordomatic.application.DownloadFileUseCase.Download;
import net.fjordomatic.application.ListMachineArchivesUseCase;
import net.fjordomatic.application.UploadFileUseCase;
import net.fjordomatic.application.ViewFileUseCase;
import net.fjordomatic.application.ViewFileUseCase.View;
import net.fjordomatic.domain.Archive;
import net.fjordomatic.domain.CannotDeleteSftpRootException;
import net.fjordomatic.domain.ConflictException;
import net.fjordomatic.domain.Excludes;
...
import net.fjordomatic.domain.Selection;
import net.fjordomatic.domain.SftpRoot;
import net.fjordomatic.domain.SourcePaths;
import net.fjordomatic.domain.Upload;
import net.fjordomatic.rest.ExplorerRestController.DirectoryResponse;
import net.fjordomatic.rest.ExplorerRestController.FileEntryResponse;
...
class ExplorerRestControllerTest {
    ...
    void get_forAMachineWithNoStoredCredential_saysSoActionably_notAGeneric500() throws Exception {
        MockMvc mockMvc = MockMvcBuilders.standaloneSetup(controller)
            .setControllerAdvice(new GlobalExceptionHandler()).build();
        // Fjord reads even its own filesystem over SSH-to-self, so the Fjord server needs a stored credential
        ...
        when(browseFilesUseCase.listDirectory(any(), any(), any()))
            .thenThrow(new NoHostCredentialException("Fjord server"));

        mockMvc.perform(get("/machines/" + mid("Fjord server") + "/files"))
            .andExpect(status().isFailedDependency())
            .andExpect(jsonPath("$.code").value("NO_CREDENTIAL"))
            .andExpect(jsonPath("$.detail").value("Fjord server"))
            .andExpect(jsonPath("$.message").value(
                "No SSH credential is stored for \"Fjord server\". Add one to browse its files."));
    }
    ...
    void view_streamsTheFileInline_withTheDomainsMediaType_andTheSafetyHeaders() throws Exception {
        ...
        // Serving fleet bytes on Fjord's own origin, to a signed-in operator: nosniff so the declared type is
    ...
    void view_ofAnHtmlFile_isRefused_asABadRequest() throws Exception {
        ...
        // bytes inline under another type. Inline HTML on Fjord's origin runs script against the session.
        when(viewFileUseCase.openForView(any(), any(), any())).thenThrow(new IllegalArgumentException(
            "Fjord will not display \"index.html\" in the browser. Download it instead."));
    ...
    void view_ofAnUppercaseSvg_isRefusedToo() throws Exception {
        MockMvc mockMvc = MockMvcBuilders.standaloneSetup(controller)
            .setControllerAdvice(new GlobalExceptionHandler()).build();
        when(viewFileUseCase.openForView(any(), any(), any())).thenThrow(new IllegalArgumentException(
            "Fjord will not display \"LOGO.SVG\" in the browser. Download it instead."));
    ...
    void downloadZip_parsesTheSelectionJson_intoCoordinates_andPassesThemToTheUseCase() throws Exception {
        ...
        org.mockito.ArgumentCaptor<List<net.fjordomatic.domain.Selection.Coordinate>> captor =
     ...
     * Fjord's heap on its way to the machine. The part is read as a stream and never as bytes — so this pins
