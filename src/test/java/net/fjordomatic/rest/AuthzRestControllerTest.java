...
import net.fjordomatic.application.AssignGroupsUseCase;
import net.fjordomatic.application.GetServiceAccessRulesUseCase;
import net.fjordomatic.application.GrantRoleUseCase;
import net.fjordomatic.application.ListAccessEntriesUseCase;
import net.fjordomatic.application.RecordAllowedAccessUseCase;
import net.fjordomatic.application.RevokeAccessUseCase;
import net.fjordomatic.application.SetServiceAccessRuleUseCase;
import net.fjordomatic.application.VerifyAccessUseCase;
import net.fjordomatic.domain.AccessDecision;
...
class AuthzRestControllerTest {
    ...
    void verify_denied_returns403WithBrandedHtmlPage() {
        ...
        // Forward-auth returns this body to the browser, so it must be the branded Fjord page.
        assertThat(response.getHeaders().getContentType().toString()).startsWith("text/html");
        assertThat(response.getBody()).contains("Fjord");
    ...
    /**
     * The one hop worth believing. A request that reached Fjord directly could put any address it liked in
    ...
    /** A request Fjord cannot even read an address off must not become a 500 on the auth path. */
    @Test
