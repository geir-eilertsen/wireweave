...
import net.fjordomatic.application.GetAccessSourcesUseCase;
import net.fjordomatic.application.GetBlockDecisionsUseCase;
import net.fjordomatic.application.GetTrustedAddressesUseCase;
import net.fjordomatic.application.GetTrustedNetworksUseCase;
import net.fjordomatic.application.LiftBlockUseCase;
import net.fjordomatic.application.TrustAddressUseCase;
import net.fjordomatic.application.UntrustAddressUseCase;
...
class SecurityRestControllerTest {
     ...
     * the read returned an empty list, and the security view rendered the most reassuring sentence Fjord
    ...
    void getDecisions_whenCrowdSecCannotBeAsked_saysSoRatherThanReportingNobodyBlocked() throws Exception {
        when(getBlockDecisions.getBlockDecisions()).thenThrow(
            new BlockDecisionsUnreadableException("Fjord could not read who CrowdSec is blocking."));

        mvc.perform(get("/security/decisions"))
            .andExpect(status().isBadGateway())
            .andExpect(jsonPath("$.code").value("BLOCK_DECISIONS_UNREADABLE"))
            .andExpect(jsonPath("$.message").value("Fjord could not read who CrowdSec is blocking."));
    }
    ...
    void deleteDecision_whenTheUnbanFails_saysSoRatherThanReportingSuccess() throws Exception {
        doThrow(new BlockNotLiftedException("Fjord could not lift the block on 1.2.3.4"))
    ...
    /** The accesses Fjord could not place are one element with nothing in it but a count. */
    @Test
