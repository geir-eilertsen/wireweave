...
class NotificationServiceTest {
    ...
    void notifyAdmins_subjectIncludesPeerNameAndNewState_disconnected() {
        ...
        assertThat(subject.getValue()).isEqualTo("[Fjord-O-Matic] file-server is now disconnected");
    }
    ...
    void notifyAdmins_subjectIncludesPeerNameAndNewState_connected() {
        ...
        ArgumentCaptor<String> subject = ArgumentCaptor.forClass(String.class);
        verify(adminNotifier).sendToAdmins(subject.capture(), any(), any());
        assertThat(subject.getValue()).isEqualTo("[Fjord-O-Matic] file-server is now connected");
    }

    @Test
    void notifyAdmins_bodyIncludesPeerDetails_andLinkToFjord() {
    ...
    void notifyAdminsOfDiskFillForecast_sendsEarlyWarning() {
        ...
        service.notifyAdminsOfDiskFillForecast(
                new net.fjordomatic.domain.DiskFillForecast("nas", "/volume1", 80, 1.0, java.time.Duration.ofHours(18)));
    ...
    void notifyAdminsOfDiskFillForecastCleared_sendsAllClear() {
        ...
        service.notifyAdminsOfDiskFillForecastCleared(
                new net.fjordomatic.domain.DiskFillForecastCleared("nas", "/volume1", 60));
    ...
    void notifyAdminsOfBackupFailure_composesSubjectAndBody() {
        ...
        ArgumentCaptor<String> body = ArgumentCaptor.forClass(String.class);
        verify(adminNotifier).sendToAdmins(subject.capture(), body.capture(), any());
        assertThat(subject.getValue()).isEqualTo("[Fjord-O-Matic] Backup failed: colina-home on Colina 27");
    ...
    void notifyAdminsOfBackupRecovery_composesAllClearSubject() {
        ...
        ArgumentCaptor<String> subject = ArgumentCaptor.forClass(String.class);
        verify(adminNotifier).sendToAdmins(subject.capture(), any(), any());
        assertThat(subject.getValue()).isEqualTo("[Fjord-O-Matic] Backup recovered: colina-home on Colina 27");
    }
    ...
    void notifyAdminsOfUpdateAvailable_composesOneRollup() {
        when(configResolver.getDomain()).thenReturn("example.com");
        net.fjordomatic.domain.ImageUpdateRollup rollup = new net.fjordomatic.domain.ImageUpdateRollup(List.of(
                new net.fjordomatic.domain.ScopedImage("Apalveien 5", "vaultwarden/server:latest"),
                new net.fjordomatic.domain.ScopedImage("Colina 27", "lscr.io/linuxserver/wireguard:1.0.x")));
