...
import net.fjordomatic.adapter.driven.InMemoryLanReachabilityCache;
...
import net.fjordomatic.domain.port.ForPingingHost;
...
class LanServerReachabilityServiceTest {
    ...
    void refreshAll_firstObservation_doesNotNotify() {
        // First time we see a server, baseline its state silently — a Fjord restart must not
    ...
    void refreshAll_warmupKeepsCacheUnknownUntilConfirmed() {
        // First couple of probes after Fjord startup must not flip the published cache —
