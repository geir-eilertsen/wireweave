...
import net.fjordomatic.application.GetOfflinePageUseCase.OfflinePage;
...
class OfflinePageServiceTest {
    ...
    void includesRetryAndDashboardLinks() {
        ...
        // Dashboard link points at the Fjord launchpad on the configured domain.
    ...
    @Test
    void emphasisesTheOInTheFjordBrand() {
        OfflinePage page = service("example.com").render(503, "bar.example.com");

        // The "-O-" of Fjord-O-Matic is set apart in its own fluorescent accent...
        assertThat(page.html()).contains("Fjord<span class=\"brand-o\">-O-</span>Matic");
        // ...backed by a .brand-o style using the fluorescent brand-o colour.
        assertThat(page.html()).contains(".brand-o");
        assertThat(page.html().toLowerCase()).contains("#39ff14");
        // The retired V-ai-er wordmark is split across a span, so a find-and-replace cannot see it.
        assertThat(page.html()).doesNotContain("brand-ai");
    }
