...
import net.fjordomatic.application.GetPublishedServicesUseCase.PublishedServiceUco;
import net.fjordomatic.application.PublishPeerServiceUseCase.PendingPublication;
import net.fjordomatic.application.PublishPeerServiceUseCase.PublishStatus;
import net.fjordomatic.application.UpdatePublishedServiceUseCase.PublishedServicePatch;
...
import net.fjordomatic.domain.*;
import net.fjordomatic.domain.DockerService.PortMapping;
import net.fjordomatic.domain.Server.State;
...
class PublishingServiceTest {
    ...
    @Mock
    ForDiscoveringFjordServerContainers discoverFjordServerContainers;

    @Mock
    ForGettingLanServerScrape getLanServerScrape;

    @Mock
    ForGettingFjordServerDockerServices getFjordServerDockerServices;
    ...
    void setUp() {
        ...
        // The Fjord server's identity: every published-service view attributes its route to a machine,
        // and a hub route belongs to the machine Fjord runs on.
        lenient().when(fjordServerIdentity.identity()).thenReturn(TestMachineIds.of("Fjord server"));
    ...
    @Test
    void getPublishedServices_routeBackedByFjordServerContainer_surfacesImageAndVersion() {
        setupOneRoute("app.example.com", "my-container", 8080);
        setupEmptyVpnClients();
        setupEmptyFjordServerServices();
        when(discoverFjordServerContainers.discover()).thenReturn(List.of(
    ...
    @Test
    void getPublishedServices_fjordInfrastructureRouter_isExcluded() {
    ...
    @Test
    void getPublishedServices_excludesOnlyTheFjordInfraRouter() {
        when(forPersistingReverseProxyRoutes.getReverseProxyRoutes()).thenReturn(List.of(
        ...
        // Only the Fjord console is mandatory infrastructure now; login + app are ordinary services.
    ...
    @Test
    void getPublishedServices_fjordServerService_nameIsSubdomainAtFjordServer() {
        ...
        assertThat(result.name()).isEqualTo("pihole @ Fjord server");
    }
    ...
    @Test
    void getPublishedServices_unknownIpAddress_nameShowsFjordServer() {
        ...
        assertThat(result.name()).isEqualTo("app @ Fjord server");
    }
    ...
    void getPublishedServices_simpleSubdomain_extractedCorrectly() {
        ...
        assertThat(result.name()).isEqualTo("traefik @ Fjord server");
    }
    ...
    /**
     * The headline contract of issue #331. Fjord supports wildcard DNS only: the operator's single
    ...
    void publishService_addressInsideServerLanCidr_dispatchesToLanFlow() {
        // A box in the Fjord server's own subnet — no relay peer needed; still routed through the
    ...
    @Test
    void deleteService_rejectsFjordService() {
    ...
    void getPublishableServices_allPeersUnreachable_returnsOnlyLocalServices() {
        ...
        PublishableService vsSvc = fjordServerService("my-app", 3000);
        when(getFjordServerDockerServices.getUnpublishedFjordServerServices(any())).thenReturn(List.of(vsSvc));
    ...
    void getPublishableServices_peerWithTcpPortNotPublished_included() {
        ...
        when(getFjordServerDockerServices.getUnpublishedFjordServerServices(any())).thenReturn(List.of());
    ...
    void getPublishableServices_mergesPeerAndLocalServices() {
        ...
        PublishableService vsSvc = fjordServerService("local-app", 3000);
        when(getFjordServerDockerServices.getUnpublishedFjordServerServices(any())).thenReturn(List.of(vsSvc));
        ...
        assertThat(result).extracting(PublishableService::source)
            .containsExactlyInAnyOrder(PublishableSource.PEER, PublishableSource.FJORD_SERVER);
    }
    ...
    void getPublishableServices_duplicates_deduplicated() {
        when(forPersistingReverseProxyRoutes.getReverseProxyRoutes()).thenReturn(List.of());
        when(discoverPeerContainers.discoverAll()).thenReturn(List.of());
        PublishableService svc = fjordServerService("my-app", 3000);
        when(getFjordServerDockerServices.getUnpublishedFjordServerServices(any()))
    ...
    void getPublishableServices_ignoredLocalService_markedIgnored() {
        when(forPersistingReverseProxyRoutes.getReverseProxyRoutes()).thenReturn(List.of());
        when(discoverPeerContainers.discoverAll()).thenReturn(List.of());
        PublishableService svc = fjordServerService("boring-app", 3000);
        when(getFjordServerDockerServices.getUnpublishedFjordServerServices(any())).thenReturn(List.of(svc));
    ...
    void getPublishableServices_nonIgnoredService_markedNotIgnored() {
        when(forPersistingReverseProxyRoutes.getReverseProxyRoutes()).thenReturn(List.of());
        when(discoverPeerContainers.discoverAll()).thenReturn(List.of());
        PublishableService svc = fjordServerService("useful-app", 3000);
        when(getFjordServerDockerServices.getUnpublishedFjordServerServices(any())).thenReturn(List.of(svc));
    ...
    @Test
    void updateService_rejectsFjordService() {
        assertThrows(IllegalArgumentException.class, () ->
    ...
    @Test
    void ignoreKey_forFjordServer_combinesContainerAndPort() {
        PublishableService s = new PublishableService(
            PublishableSource.FJORD_SERVER, TestMachineIds.of("Fjord server").value(), null,
    ...
    private void setupEmptyFjordServerServices() {
    ...
    private PublishableService fjordServerService(String name, int port) {
        return new PublishableService(PublishableSource.FJORD_SERVER,
            TestMachineIds.of("Fjord server").value(), null, name, name, port, null, false);
    }
