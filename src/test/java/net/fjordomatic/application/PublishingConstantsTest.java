...
class PublishingConstantsTest {
    ...
    @Test
    void isMandatory_trueForTheFjordServerFqdn() {
    ...
    void isMandatory_trueForTheOauth2ProxyFqdn() {
        // oauth2.<domain> is Fjord infrastructure (the sign-in gateway), never an ordinary service.
        assertThat(PublishingConstants.isMandatory("oauth2.example.com", "example.com")).isTrue();
    }

    @Test
    void isMandatory_trueForTheDexBrokerFqdn() {
        // dex.<domain> is Fjord infrastructure (the OIDC broker), never an ordinary service.
    ...
    void isMandatory_falseForTheDecommissionedAutheliaLoginFqdn() {
        // Authelia is decommissioned; login.<domain> is no longer Fjord infrastructure.
