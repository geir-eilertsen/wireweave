...
/**
 * The contents that survive Fjord — what a person reads after decrypting a {@link SurvivalKit}.
 *
 * <p>Everything Fjord knows about reading its own backups is, today, inside Fjord: the repository
 ...
 * losing the Fjord server leaves an encrypted repository whose passphrase is inside itself. This is the way
 * out of that circle, and it only works if a copy lives somewhere Fjord does not.
 *
...
class RecoverySheetTest {
    ...
    @Test
    void itCarriesTheCommandThatReadsThem_soNoFjordIsNeededToRecover() {
    ...
    @Test
    void itCarriesTheConfigKeyToo_becauseARestoredFjordCannotReadItsOwnSecretsWithoutIt() {
        // The archives open with the passphrases alone. But restoring Fjord itself from one of them yields a
    ...
    void aRepositoryNamedByIdentity_isStillReadAsItsMachine() {
        ...
        // to a person. This page is where that mapping lives once Fjord is gone — it has to carry both, the
