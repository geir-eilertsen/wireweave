...
class BackupJobTest {
    ...
    void protecting_aPathInsideAnExcludedFolder_dropsThatWiderExcludeToo() {
        // The exclude is an ancestor of what was just explicitly asked for. Fjord cannot exclude and protect
    ...
    // --- isDue: Fjord-owned nightly scheduling decision (Slice 6) ---
    ...
    void readyClientHostForFirstBackup_readiesThroughThePortAndReturnsTheOutcome_whenFirst() {
        ...
        var readier = org.mockito.Mockito.mock(net.fjordomatic.domain.port.ForReadyingBackupClients.class);
        var expected = new net.fjordomatic.domain.port.ForReadyingBackupClients.ReadyingOutcome(
        ...
        Optional<net.fjordomatic.domain.port.ForReadyingBackupClients.ReadyingOutcome> result =
    ...
    void enablingBackupAsRoot_asksForTheGrantAndLeavesTheFlagOffWhenTheMachineDoesNotHaveIt() {
        // The grant is installed asynchronously, so Fjord cannot yet say it landed. Turning the flag on now
