...
/**
 * The kit that outlives the Fjord server.
 *
 ...
 * Fjord can rewrite — an encrypted file it distributes across the fleet. What the operator keeps shrinks from
 ...
 * lives on machines that may be compromised), and opening it needs nothing but {@code openssl} (needing Fjord
 * to read Fjord's own recovery kit would restore the exact circularity it exists to break).
 */
class SurvivalKitTest {
    ...
    void itSaysWhenItWasWritten_soAKitThatStoppedBeingUpdatedCanBeSeenToHaveStopped() {
        ...
        // decrypting, on a machine that no longer has a Fjord to ask.
