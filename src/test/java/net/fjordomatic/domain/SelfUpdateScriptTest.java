...
/**
 * The script Fjord runs on its own host to replace itself.
 *
...
class SelfUpdateScriptTest {
    ...
    void itLeavesItsOwnAccountOnTheHost_becauseNobodyIsListeningWhenItFinishes() {
        // Fjord is restarting while this runs, so there is no in-memory state to settle against and no open
        // connection to report to: the result has to outlive both processes. Fjord reads this file when it
        // comes back up.
        String script = SelfUpdateScript.generate(DIR, "vaier", "run-1", 90);
    ...
    void theComposeProjectDirectoryIsQuoted() {
        // The directory is operator-configurable and lands in a shell command. Fjord quotes every path it
