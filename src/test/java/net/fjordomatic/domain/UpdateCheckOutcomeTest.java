...
/**
 * What Fjord says it did when the operator asked it to check (#57 slice 3).
 *
...
class UpdateCheckOutcomeTest {
    ...
    private static ScopedImage si(String image) {
        return new ScopedImage("Fjord server", image);
    }

    @Test
    void aCheckThatChangedNothing_saysSo_butStillSaysItChecked() {
        // The commonest outcome by far, and the one the UI must not dress up. The operator pulled, Fjord
    ...
    void aCoalescedCheck_neverClaimsToHaveChecked() {
        // THE honesty rule of the floor. Fjord refused to ask the registries, so it does not get to say it
