...
class ContainerUpdateEligibilityTest {
    ...
    private static ContainerUpdateEligibility onFjordServer(DockerService container) {
        return onFjordServer(container, DockerCommandAccess.GRANTED);
    }

    private static ContainerUpdateEligibility onFjordServer(DockerService container, DockerCommandAccess access) {
        return ContainerUpdateEligibility.judgeFjordServerContainers(List.of(container), access)
    ...
    void aComposeManagedContainerOfTheOperatorsIsUpdatable() {
        ...
        assertThat(onFjordServer(container("vaultwarden", COMPOSE_LABELS)))
            .isEqualTo(ContainerUpdateEligibility.UPDATABLE);
    }
    ...
    @Test
    void aContainerOnAMachineWhoseDockerFjordCannotDriveIsNotUpdatable() {
        // Colina 27: the scrape reads Docker's API over the tunnel and needs no group, so the machine looks
        // healthy while every `docker compose` Fjord would run there dies on permission denied. Withhold
        ...
        assertThat(onFjordServer(container("pihole", COMPOSE_LABELS), DockerCommandAccess.REFUSED))
            .isEqualTo(ContainerUpdateEligibility.NO_DOCKER_ACCESS);
    }
    ...
    void aMachineNobodyHasSweptYetKeepsTheButton_becauseUnknownIsNotNo() {
        ...
        // coordinates, and the difference is the evidence. Missing coordinates mean Fjord genuinely does
    ...
    void vaiersOwnStackOutranksEverything_becauseTheReasonIsNotAboutTheHostAtAll() {
        assertThat(onFjordServer(container("wireguard", COMPOSE_LABELS), DockerCommandAccess.REFUSED))
            .isEqualTo(ContainerUpdateEligibility.FJORD_OWN_STACK);
    }
    ...
    void theRefusalNamesTheRemedy_notOnlyTheFault() {
        ...
        // stays generic about WHICH user — the Explorer knows which user Fjord acts as on each machine.
    ...
    void aContainerWithoutComposeCoordinatesIsNotUpdatable() {
        // Started with plain `docker run`: Fjord cannot recreate it without knowing how it was started,
    ...
    @Test
    void vaiersOwnStackOnTheFjordServerIsNotUpdatable() {
        // Version-pinned by a Fjord release; a per-container button would be a second, conflicting
        // update path for the same images.
        assertThat(onFjordServer(container("wireguard", COMPOSE_LABELS)))
            .isEqualTo(ContainerUpdateEligibility.FJORD_OWN_STACK);
        assertThat(onFjordServer(container("vaier", COMPOSE_LABELS)))
            .isEqualTo(ContainerUpdateEligibility.FJORD_OWN_STACK);
    }

    @Test
    void anOfferedContainerOfFjordsOwnStackIsStillFjordsOwn() {
        // Traefik is offered for publishing rather than excluded — offered says nothing about whose
        // container it is, and it is still pinned by a Fjord release.
        assertThat(onFjordServer(container("traefik", COMPOSE_LABELS)))
            .isEqualTo(ContainerUpdateEligibility.FJORD_OWN_STACK);
    }

    @Test
    void thatVerdictIsScopedToTheFjordServerAlone() {
        // The catalogue is about containers running on the Fjord server. A peer or LAN server running its
    ...
    void vaiersOwnStackIsRefusedEvenWhenItsLabelsWereUnreadable() {
        ...
        assertThat(onFjordServer(container("oauth2-proxy", Map.of())))
            .isEqualTo(ContainerUpdateEligibility.FJORD_OWN_STACK);
    }

    @Test
    void onlyUpdatableSaysFjordMayOfferTheAction() {
        ...
        assertThat(ContainerUpdateEligibility.FJORD_OWN_STACK.updatable()).isFalse();
    }
    ...
    void judgingAnEmptyScrapeIsAnEmptyList() {
        ...
        assertThat(ContainerUpdateEligibility.judgeFjordServerContainers(List.of(), DockerCommandAccess.GRANTED)).isEmpty();
    }
}
