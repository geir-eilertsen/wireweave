...
class MachineTest {
    ...
    void twoMachinesMayShareAName_andAreStillDifferentMachines() {
        ...
        // and two houses may each have a "NAS" without Fjord ever confusing one for the other.
    ...
    @Test
    void fjordServer_hasCanonicalNameServerCategory_andDefaultsSshOn() {
        Machine m = Machine.fjordServer(MachineId.generate(), null);

        assertThat(m.name()).isEqualTo(LanAnchor.FJORD_SERVER_NAME);
    ...
    @Test
    void fjordServer_runsDocker() {
        // The Fjord server host is itself a Docker engine — it runs the whole compose stack (WireGuard,
        // Traefik, oauth2-proxy, Fjord). So its Machine projection must say it runs Docker, or the Explorer
        // tree never grows a `containers` entry for it and its own containers stay invisible.
        assertThat(Machine.fjordServer(MachineId.generate(), null).runsDocker()).isTrue();
    }

    @Test
    void fjordServer_honoursExplicitOverride() {
        assertThat(Machine.fjordServer(MachineId.generate(), false).effectiveSshAccess()).isFalse();
        assertThat(Machine.fjordServer(MachineId.generate(), true).effectiveSshAccess()).isTrue();
    }
