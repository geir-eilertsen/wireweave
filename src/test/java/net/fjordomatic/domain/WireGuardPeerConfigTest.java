...
class WireGuardPeerConfigTest {
    ...
    // The server LAN CIDR is the subnet the Fjord server itself sits on. Appending it to a
    ...
    void generate_descriptionWithNewline_isEscapedToStayOnOneLine() {
        ...
        String fjordLine = config.lines().filter(l -> l.startsWith("# VAIER:")).findFirst().orElseThrow();
        assertThat(fjordLine).contains("line one\\nline two");
        assertThat(fjordLine).doesNotContain("\n");
    }
    ...
    void generate_nameWithQuotesAndBackslashes_isJsonEscaped() {
        ...
        String fjordLine = config.lines().filter(l -> l.startsWith("# VAIER:")).findFirst().orElseThrow();
        assertThat(fjordLine).contains("\\\"box\\\"");
        assertThat(fjordLine).contains("C:\\\\data");
    }
    ...
    @Test
    void isOutOfDate_falseWhenOnlyFjordMetadataCommentDiffers() {
        // The "# VAIER:" comment is pure Fjord-side metadata — never installed into the tunnel.
    ...
    @Test
    void fjordJson_appendsDeviceCategoryLastWhenOverridePresent() {
        String json = WireGuardPeerConfig.fjordJson(
    ...
    @Test
    void fjordJson_blankDeviceCategory_omitsKey() {
        String json = WireGuardPeerConfig.fjordJson(
    ...
    @Test
    void generate_writesTheMachineIdIntoTheFjordMetadata() {
        ...
        // to the WireGuard server and then is invisible to Fjord: no machine, no credential, no backup.
