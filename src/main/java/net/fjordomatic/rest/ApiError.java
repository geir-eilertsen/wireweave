 ...
 * {@code SettingsRestController} also emits it directly, where bad SMTP credentials
 * are deliberately mapped to {@code 400} rather than the generic {@code 500} the handler
 * would give a raw library exception. Validation ({@code 400}), not-found ({@code 404}, via
 * {@link NotFoundException}) and conflict ({@code 409}, via
 * {@link ConflictException}) failures all render in this envelope. The
 * only error responses that aren't an {@code ApiError} are the body-less {@code 404}s
 * for missing optional GET artifacts (icon, an
