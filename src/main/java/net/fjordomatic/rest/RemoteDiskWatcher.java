...
import net.fjordomatic.domain.RemoteDiskForecastTracker;
import net.fjordomatic.domain.RemoteDiskPressureTracker;
 ...
 * still its largest job, but what it really is now is <em>the one trip Fjord makes to every credentialed
 * machine</em>, and four things are learned on it: each machine's filesystems (and the alerts they raise),
 * its {@link net.fjordomatic.domain.SshServerPresence}, the network it sits on, and — since #352 — its
 ...
 *
 * <p>Polls <b>every real filesystem</b> of every SSH-accessible machine that Fjord holds a credential for
 * and emails admins when a watched one crosses its alert threshold. This covers the Fjord host itself (via
 ...
 * {@link net.fjordomatic.domain.DiskPressureBand}, and once more when it drops back to normal. Nothing is
 * re-alerted every poll, and nothing is re-alerted on a timer either.
 *
 * <p><b>The silence this watcher was fixed for.</b> The Fjord server's own root filesystem reached 89%
 ...
 * NAS that is the fixed-size 2.3 GB DSM system partition, 88% by design and never moving, so the alert Fjord
 ...
 *
 * <p>Machines without SSH access or without a stored credential are skipped silently, so Fjord never mounts a
 ...
 * credential), which is precisely why "a machine Fjord cannot read is simply not nudged" needs no rule of
 * its own.
 */
@Component
@Slf4j
public class RemoteDiskWatcher {
    ...
    // A second fact this same trip can teach, and the only place Fjord can learn it: the container scrape
    ...
    public void checkRemoteDiskUsage() {
        ...
        // A machine deleted while Fjord was running must not leave its SSH-server presence — or the network
    ...
    /**
     * One machine, behind the two guards that decide whether Fjord may reach it at all. Both consumers of
     * this trip sit behind them, and that is what makes the acceptance criterion of #333 — "a machine Fjord
    ...
    private void checkDisks(Machine machine, DiskWatches watches, int globalThreshold) {
        try {
            ...
            // Recorded before df's own exit status is judged: whether Fjord can drive Docker there is a fact
