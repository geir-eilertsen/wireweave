...
import net.fjordomatic.application.SendHostPasswordUseCase.SendPasswordResult;
 ...
 * the upgrade) — Fjord does no in-process auth here.
 */
@Slf4j
...
public class TerminalWebSocketHandler extends AbstractWebSocketHandler {
     ...
     * laptop lid or a Fjord redeploy — so the remote tmux session is deliberately left alive for the reconnect
     ...
     * contain spaces, e.g. "Fjord server"). Null when the path doesn't match.
     */
     ...
     * still register.
     */
    private static final class WsOutputListener implements net.fjordomatic.domain.port.ForOpeningSshSessions.SshOutputListener {
