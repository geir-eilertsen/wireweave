...
import net.fjordomatic.domain.RecoverySheet;
import net.fjordomatic.domain.SurvivalKit;
import net.fjordomatic.domain.SurvivalKitFreshness;
...
public class SurvivalKitWriter implements WriteSurvivalKitUseCase {
    ...
    private final GetFjordServerUseCase fjordServer;
    ...
    public SurvivalKitWriter(GetMachinesUseCase machines,
                             GetFjordServerUseCase fjordServer,
    ...
    /**
     * How often Fjord checks that what the fleet is holding still says what Fjord knows. Ten minutes: the
    ...
    public SurvivalKitReport writeSurvivalKit() {
        FjordConfig config = configuration.load().orElseGet(() -> FjordConfig.builder().build());
        if (!config.hasSurvivalKitPassphrase()) {
            ...
            // chose the passphrase for. Fjord will not invent one: a kit Fjord could open dies with Fjord.
            throw new IllegalStateException(
                "Choose a survival kit passphrase before writing a kit — Fjord will not invent one, "
    ...
    /**
     * Keep the fleet's kits saying what Fjord knows, without being asked.
     *
     ...
     * the failure mode would be Fjord silently stopping watching, which is the one this exists to prevent.
     */
    ...
    public void keepTheFleetsKitsCurrent() {
        try {
            ...
            log.info("The fleet's survival kits no longer say what Fjord knows — writing them again");
            write(config, inputs);
    ...
    private SurvivalKitReport write(FjordConfig config, KitInputs inputs) {
        SurvivalKitHosts.Selection selection =
            SurvivalKitHosts.select(inputs.fleet(), fjordServer.getFjordServerMachine().name(), geo);
        ...
        } else if (rollout.survivesLossOfFjord()) {
    ...
    /** Remember what the fleet is now holding, on disk, so the answer survives a restart of Fjord. */
    private void remember(String fingerprint) {
        FjordConfig fresh = configuration.load().orElseGet(() -> FjordConfig.builder().build());
