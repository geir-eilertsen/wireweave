...
import net.fjordomatic.application.CreatePeerUseCase;
import net.fjordomatic.application.DeletePeerUseCase;
import net.fjordomatic.application.GenerateDockerComposeUseCase;
import net.fjordomatic.application.GeneratePeerSetupScriptUseCase;
import net.fjordomatic.application.GetPeerConfigUseCase;
import net.fjordomatic.application.GetServerLocationUseCase;
import net.fjordomatic.application.GetVpnPeersUseCase;
import net.fjordomatic.application.GetVpnPeersUseCase.VpnPeerView;
import net.fjordomatic.application.ReissuePeerConfigUseCase;
import net.fjordomatic.application.RenamePeerUseCase;
import net.fjordomatic.application.UpdateLanCidrUseCase;
import net.fjordomatic.application.UpdatePeerDeviceCategoryUseCase;
...
import net.fjordomatic.domain.port.ForTrackingPeerConfigRetrieval;
import net.fjordomatic.domain.port.ForUpdatingPeerConfigurations;
import net.fjordomatic.domain.port.ForVendingSetupTokens;
import net.fjordomatic.config.ServiceNames;
import net.fjordomatic.domain.MachineIntent;
...
public class VpnPeerRestController {
    ...
    private CreatePeerResponse buildConfigDeliveryResponse(String id, MachineId machineId, String name,
        ...
        java.util.Set<net.fjordomatic.domain.PeerArtifact> artefacts =
            net.fjordomatic.domain.PeerArtifact.forPeerType(peerType);

        String qrCodePngBase64 = artefacts.contains(net.fjordomatic.domain.PeerArtifact.QR_CODE)
            ? tryEncodeQrCodeBase64(configFile, name)
            : null;
        String dockerCompose = artefacts.contains(net.fjordomatic.domain.PeerArtifact.DOCKER_COMPOSE)
            ? generateDockerComposeUseCase.generateWireguardClientDockerCompose(
                id, defaultServerUrl(), ServiceNames.DEFAULT_WG_PORT)
            : null;
        String setupScript = artefacts.contains(net.fjordomatic.domain.PeerArtifact.SETUP_SCRIPT)
    ...
    public ResponseEntity<?> getPeerConfig(@PathVariable String peerIdentifier) {
        ...
        PeerConfigResponse response = new PeerConfigResponse(
                ...
                net.fjordomatic.domain.PeerArtifact.forPeerType(result.peerType()).stream()
     ...
     * single-use {@link net.fjordomatic.domain.SetupToken} is the only authorization, validated here in
     * Fjord. The token is per-peer, ~15-min TTL, and burned on first use; it also burns the one-shot
    ...
    public ResponseEntity<?> downloadTokenizedSetupScript(
        ...
        if (token == null || !forVendingSetupTokens.consume(peerId, token)) {
            ...
            return ResponseEntity.status(org.springframework.http.HttpStatus.UNAUTHORIZED)
                ...
                .body("This setup link is invalid or has already been used. Regenerate it in Fjord.\n");
        }
        ResponseEntity<?> gate = checkOneShotGate(peerId);
        if (gate != null) return gate;
    ...
    private String defaultServerUrl() {
        return new net.fjordomatic.domain.FjordHostnames(configResolver.getDomain()).fjordServerFqdn();
    }
