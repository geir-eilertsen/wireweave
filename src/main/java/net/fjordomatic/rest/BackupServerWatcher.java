...
import net.fjordomatic.domain.BackupServerHealthTracker;
import net.fjordomatic.domain.Machine;
import net.fjordomatic.domain.port.ForProbingTcp;
 ...
 * back). Fjord owns fleet backup, so a vanished Backup server deserves its own alert rather than surfacing
 ...
 *
 * <p><b>What this watch does and does not answer.</b> It probes from <em>Fjord</em>, so it answers
 * "is the backup server up?" — <em>not</em> "can backup jobs reach it?". A backup client on another LAN can
 * fail to route to a server that Fjord sees perfectly well (this asymmetry was the live Colina routing bug).
