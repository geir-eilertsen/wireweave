...
import net.fjordomatic.domain.LastAdminException;
import net.fjordomatic.domain.NoHostCredentialException;
import net.fjordomatic.domain.NoSftpSubsystemException;
...
import net.fjordomatic.domain.PathOutsideSftpRootException;
import net.fjordomatic.domain.PermissionDeniedException;
 ...
 *
 * <p>Domain validation throughout Fjord signals bad input by throwing
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    /**
     * Fjord could not read a repository's archives. {@code 502}, not {@code 500}: the failure is upstream —
     * borg, on the machine, talking to the backup server — and Fjord is relaying its words rather than
    ...
    /**
     * The <em>machine</em> refused the read, not Fjord — the SSH user is not allowed to see that path. This is
     * an ordinary state of a fleet whose SSH users are deliberately not root, so it must read as "you cannot
     * read this", never as a Fjord fault.
     */
    ...
    /**
     * A machine with no stored SSH credential is a configuration gap the operator must close, not a Fjord
     ...
     * logged server-side but never returned; the operator is told to check the credential Fjord holds instead.
     */
    ...
    public ResponseEntity<ApiError> handleSshAuth(SshAuthException e) {
        ...
        return ResponseEntity.status(HttpStatus.BAD_GATEWAY)
                .body(ApiError.of("SSH_AUTH_FAILED", "The SSH credential Fjord holds was rejected — check it."));
    }

    /**
     * Fjord reached for a machine's disk and could not read it — an asleep machine, a {@code df} that
     * exited non-zero, output that will not parse. Mapped explicitly to {@code 502} so it carries its own
     * sentence ("Fjord could not read the disk on ...") instead of falling through to the generic
     * {@code 500} "An unexpected error occurred", which would tell the operator nothing they can act on.
     * The gateway status is the honest one: the fault is on the far side of Fjord, not in it.
     */
     ...
     * {@code 502}: the far side answered, so the fault is neither Fjord's nor the operator's request; it is a
     ...
     * Registered explicitly: unmapped, this was the generic {@code 500} that made Fjord look broken while
    ...
    /**
     * Fjord could not reach a machine over SSH at all — unreachable, or a handshake that timed out.
     * {@code 502} for the same reason an unreadable disk is: the failure is on the far side of Fjord. The
    ...
    /**
     * The machine presented a host key that is not the one Fjord pinned. The web terminal has always shown
    ...
    /**
     * Fjord asked CrowdSec to let an address back in and could not tell that it had (#329 Slice 3c).
     * {@code 502} like every other far-side refusal: the engine that owns the ban is on the other side of
     * Fjord. Mapped explicitly, and carrying the domain's own sentence, because the generic {@code 500}
     * would read as "Fjord is broken" to an operator whose real question is the far narrower "am I back
    ...
    /**
     * Fjord could not find out who CrowdSec is currently keeping out (#329 Slice 3). {@code 502} like its
     * sibling above: the engine that owns the bans is on the far side of Fjord.
     *
     * <p>What this replaces is worse than a wrong status code. The read used to swallow its failures and
     * answer {@code 200 []}, which the security view renders as "Nobody is blocked right now" — so a Fjord
