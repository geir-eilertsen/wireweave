 ...
 *       <em>without</em> the Fjord console's own auth middleware (it is the auth check itself).
 ...
 *       behind the existing Fjord console auth.</li>
 * </ul>
 */
@RestController
@Slf4j
public class AuthzRestController {
     ...
     * identical question — which hop of {@code X-Forwarded-For} came from Fjord's own Traefik — so an
    ...
    private static String loadDeniedPage() {
        try (InputStream in = AuthzRestController.class.getResourceAsStream("/authz-denied.html")) {
            if (in == null) {
                return "<!DOCTYPE html><title>Awaiting approval · Fjord-O-Matic</title>"
