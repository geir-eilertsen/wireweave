...
import net.fjordomatic.application.ProbeLanHostUseCase;
import net.fjordomatic.application.RegisterLanServerUseCase;
import net.fjordomatic.application.RegisterLanServerUseCase.RegistrationOutcome;
...
import net.fjordomatic.domain.LanHostProbe;
import net.fjordomatic.domain.LanMachineRole;
...
public class LanServerRestController {
    ...
    /**
     * What routes packets to {@code address}: a relay peer's lanCidr, or the Fjord server itself
     ...
     * {@link SetupToken} is the only authorization, validated here in Fjord. The script carries no
     ...
     * {@link DeviceCategory#detect}); {@code routedVia} is the relay (or the Fjord server) that reaches it.
    ...
    /** {@code routedVia} is a relay peer name or "Fjord server"; both null when not routable. */
    ...
    record LanServerResponse(
        ...
        static LanServerResponse from(LanServerView view, String reachability, MachineStatus status, Long lastSeen) {
            return new LanServerResponse(
                ...
                view.relayPeerName(),
                view.relayMachineId() == null ? null : view.relayMachineId().value(),
                reachability,
                status,
                lastSeen,
