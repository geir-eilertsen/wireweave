...
/**
 * Admin CRUD for the one host credential Fjord holds per machine (the credential vault, #307). These
...
public class HostCredentialRestController {
    ...
    /**
     * Mint a managed keypair for this machine (#309). Destructive — it replaces whatever login Fjord held
    ...
    /** Only the login name: everything else about a managed keypair is Fjord's decision, not the caller's. */
