...
import net.fjordomatic.domain.BackupFleet;
import net.fjordomatic.domain.BackupJob;
import net.fjordomatic.domain.BackupRun;
import net.fjordomatic.domain.EffectiveUser;
...
import net.fjordomatic.domain.MachineNudges;
import net.fjordomatic.domain.MachineSignals;
...
public class MachineRestController {
    ...
    private final GetFjordServerUseCase getFjordServerUseCase;
    ...
    /**
     * Every machine Fjord knows. Each carries {@code hasCredential} — whether Fjord actually holds an SSH
     * login for it — composed here at the driving edge from {@link GetHostCredentialUseCase}, exactly as the
     * Fjord-server and nudges endpoints do. The Explorer gates a machine's Files and Disk entries on it:
     * those ride on a credential, so the SSH-access toggle alone would grow entries that open onto a "no
     * login" wall.
     *
     * <p>Each also carries {@code sshServerPresence} — Fjord's last-known belief about whether an SSH server
     ...
     * Fjord acts as there, and whether that user is privileged; the username is null where no credential is
    ...
    public List<MachineResponse> list() {
        // Which machine Fjord is running on, as an identity. The browser used to work this out by
        // comparing a display name to the literal "Fjord server", which is a name doing an identity's
        // job: rename the host and Fjord stops recognising itself; let another machine take the name
        ...
        // a lookup can fail — resolving the Fjord server reads config and shells into the WireGuard
        ...
        MachineId fjordServer = resolveFjordServerId();
        return getMachinesUseCase.getAllMachines().stream()
            .map(m -> {
                ...
                // login is held at all, and which user that login makes Fjord on this machine. Asking
    ...
    /** The Fjord server's identity, or null when it cannot be resolved right now. Never throws. */
    private MachineId resolveFjordServerId() {
        try {
            Machine server = getFjordServerUseCase.getFjordServerMachine();
            return server == null ? null : server.id();
        } catch (RuntimeException e) {
            log.warn("Could not resolve the Fjord server's identity for the machine list: {}", e.getMessage());
            return null;
        }
    }
     ...
     * definition of "Fjord holds a usable login here"; two would be free to drift.
     */
    ...
    /**
     * The Fjord server host as a machine (#311): its canonical name, effective SSH access, and
     * whether a host credential is stored for it. Feeds the dedicated Fjord-server card's SSH-access
    ...
    public FjordServerResponse fjordServer() {
        Machine server = getFjordServerUseCase.getFjordServerMachine();
        return new FjordServerResponse(server.id().value(), server.name(), server.effectiveSshAccess(),
            hasStoredCredential(server.id()));
    }

    record FjordServerResponse(String id, String name, boolean sshAccess, boolean hasCredential) {}

    /**
     * The progressive-adoption nudges Fjord suggests for one machine: evidence-backed, single yes/no
    ...
    public List<NudgeResponse> nudges(@PathVariable String machineId) {
        ...
        // name — the owner's name matched against the machine's — which needed a Fjord-server name and an
        ...
        // #333: the networks the scheduled sweep last read off this machine, and off the Fjord server —
        ...
        MachineId fjordServer = resolveFjordServerId();
        MachineNetworks routingHostNetworks = fjordServer == null ? MachineNetworks.unknown()
            : getMachineNetworksUseCase.getMachineNetworks(fjordServer);
    ...
    /**
     * Sets whether Fjord offers SSH for a machine — the credential control now, the web terminal
     ...
     * Fjord watches it, and the domain's verdict on it. The verdict travels rather than the browser
    ...
    public record MachineResponse(
        ...
        static MachineResponse from(Machine m, boolean hasCredential, boolean fjordServer,
                                    SshServerPresence sshServerPresence) {
            return from(m, hasCredential, fjordServer, sshServerPresence, null);
        }

        /**
         * @param effectiveUser the user Fjord acts as on this machine, or null when it holds no login for
