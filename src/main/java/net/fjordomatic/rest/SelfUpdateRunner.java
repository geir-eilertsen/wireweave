...
import net.fjordomatic.application.GetSelfUpdateStatusUseCase;
import net.fjordomatic.application.RunRemoteCommandUseCase;
import net.fjordomatic.application.UpdateFjordUseCase;
...
import net.fjordomatic.domain.SelfUpdate;
...
import net.fjordomatic.domain.SelfUpdateScript;
import net.fjordomatic.domain.SelfUpdateStatus;
import org.springframework.stereotype.Component;

/**
 * Carries out Fjord's self-update on its own host.
 *
 ...
 * work, not a domain decision. Every decision it makes is asked of the domain — which container is Fjord
 ...
 *
 * <p><b>SSH to its own host.</b> The same channel {@code RemoteDiskWatcher} uses to read the Fjord server's
...
@Component
@Slf4j
public class SelfUpdateRunner implements UpdateFjordUseCase, GetSelfUpdateStatusUseCase {

    private final DiscoverFjordServerContainersUseCase containers;
    private final RunRemoteCommandUseCase remoteCommand;
    private final ForResolvingFjordServerIdentity fjordServerIdentity;
    // Settings speaks for itself (#353): Fjord's own stack is no longer swept, so the one image this may
    ...
    public SelfUpdateRunner(DiscoverFjordServerContainersUseCase containers,
    ...
    /**
     * The Fjord server's own identity. It is the one machine that cannot be found by searching the machine
    ...
    private MachineId fjordServerId() {
        return fjordServerIdentity.identity();
    }
    ...
    public boolean updateAvailable() {
        try {
            ...
            log.debug("Could not judge whether a Fjord update is available: {}", e.getMessage());
            return false;
        }
    }
    ...
    public SelfUpdateStatus lastUpdate() {
        try {
            CommandResult result = remoteCommand.run(fjordServerId(), SelfUpdateScript.readResult());
     ...
     * Fjord down: without its own container it does not know what to recreate, and without compose labels
     * there is no {@code docker compose up} that would bring it back — an update there would stop Fjord and
     * leave it stopped.
     */
    @Override
    public SelfUpdateStatus updateSelf() {
        ...
        try {
            CommandResult labels = remoteCommand.run(fjordServerId(),
                SelfUpdateScript.inspectComposeLabels(self.get().containerId()));
            at = SelfUpdateScript.parseComposeLabels(labels.stdout());
        } catch (Exception e) {
            log.warn("Could not reach the Fjord host to update: {}", e.getMessage());
        ...
        try {
            CommandResult staged = remoteCommand.run(fjordServerId(),
            ...
            // can be relied on to run — which is why the script, not Fjord, decides how the update ends.
            remoteCommand.run(fjordServerId(), SelfUpdateScript.launch(at.get().workingDir(), runId));
            log.info("Fjord self-update {} launched on its own host", runId);
            return new SelfUpdateStatus(runId, SelfUpdateStatus.Outcome.NONE, null, "started");
        } catch (Exception e) {
            log.warn("Launching the Fjord self-update failed: {}", e.getMessage());
            return failed(runId, "launch-failed");
        }
    }

    private SelfUpdateStatus failed(String runId, String why) {
        log.warn("Fjord self-update {} refused: {}", runId, why);
