...
/**
 * A host running a borg server ({@code borg serve}) that Fjord can address over SSH, and on which one or
 ...
 *
 * @param name           the Fjord-facing identity of this server, unique within the store
 * @param machineId      the identity of the Fjord {@link Machine} hosting it; how Fjord SSHes to it. Held as
 ...
 * @param managed        {@code true} when Fjord provisioned this server, {@code false} when it was adopted
 */
public record BackupServer(
     ...
     * ({@code <serverDataPath>/ssh/host_keys.pub}). Fjord reads it from the server's machine and installs the
     ...
     *
     * <p>Note: this alert answers "is the backup server up?" as seen <em>from Fjord</em> — it is not a claim
    ...
    public String downBody(String machineLabel, String baseDomain, ProbeResult cause) {
        ...
        appendFjordLink(body, baseDomain);
    ...
    private void appendFjordLink(StringBuilder body, String baseDomain) {
