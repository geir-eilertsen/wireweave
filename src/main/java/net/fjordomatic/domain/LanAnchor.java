 ...
 * {@code lanCidr} contains it, or — failing that — the Fjord server itself, when the address
 * falls inside the Fjord server's own LAN CIDR ({@code ForResolvingServerLanCidr}).
 *
 ...
 * LAN service anchored at the Fjord server is reachable directly from the Fjord-side containers
 * (vaier / traefik → docker bridge → host → the host's LAN/VPC NIC), so no relay peer is needed.
 */
public final class LanAnchor {

    /** Canonical display name for the Fjord server itself, used wherever a peer name would otherwise appear. */
    public static final String FJORD_SERVER_NAME = "Fjord server";

    private final PeerConfiguration relayPeer; // null ⇒ the Fjord server
    ...
    /**
     * Every LAN Fjord can sweep for unregistered machines: each relay peer that routes a
     * {@code lanCidr}, in config order, followed by the Fjord server's own LAN when its CIDR is
     ...
     * {@link #FJORD_SERVER_NAME} for the server LAN — resolved to its CIDR. Empty when no scannable
    ...
    public boolean isFjordServer() {
        return relayPeer == null;
    }

    /** The relay peer when {@link #isFjordServer()} is false; otherwise empty. */
    public Optional<PeerConfiguration> relayPeer() {
        return Optional.ofNullable(relayPeer);
    }

    /** Relay peer name, or {@link #FJORD_SERVER_NAME} when anchored at the Fjord server. */
    public String name() {
        return relayPeer == null ? FJORD_SERVER_NAME : relayPeer.name();
    }
     ...
     * host} is tagged with), or {@link #FJORD_SERVER_NAME} for the server LAN. Distinct from {@link
    ...
    public String anchorKey() {
        return relayPeer == null ? FJORD_SERVER_NAME : relayPeer.id();
    }

    /**
     * The identity of the machine that routes to this LAN — the relay peer's {@link MachineId}, or empty
     * when the LAN is the Fjord server's own (it is not a peer, and the caller already knows which machine
     * it is standing on).
     *
