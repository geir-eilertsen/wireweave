 ...
 *
 * @param publishableCount    services exposed on the machine but not yet routed through Fjord
 * @param reachable           whether the machine is reachable right now (from cached signals)
 * @param hasCredential       whether Fjord already holds an SSH credential for the machine
 ...
 * @param networks            what Fjord last read off this machine's own interfaces (#333)
 * @param routingHostNetworks what Fjord last read off the host that would install a LAN route — the
 *                            Fjord server — so a network that would sever its uplink is never offered
 */
@Builder
