 ...
 * discovered (Fjord server, server peer, or Docker-enabled LAN server). Owns the publishing
...
public record PublishableService(
    ...
    public enum PublishableSource { FJORD_SERVER, PEER, LAN_SERVER }
    ...
    public String ignoreKey() {
        return switch (source) {
            ...
            case FJORD_SERVER -> containerName + ":" + port;
        };
    }
    ...
    /**
     * The subdomain the publish modal pre-fills. Fjord-server services get the container name
    ...
    public String suggestedSubdomain() {
        return source == PublishableSource.FJORD_SERVER
            ? containerName
