...
public class FjordConfig {
    ...
    /**
     * The hour of day (0–23, in the Fjord server's clock zone) at which Fjord-owned nightly fleet-backup
    ...
    /**
     * Whether Fjord offers SSH for the Fjord-server host itself (#311) — the explicit operator
     * override, or null when unset (the effective value then falls back to the server default: on).
     * The Fjord server is neither a peer nor a LAN server, so its SSH-access override lives here.
     */
    private Boolean fjordServerSshAccess;

    /**
     * The {@link MachineId} of the Fjord-server machine (#311), as its canonical string. The Fjord
     * server is neither a peer nor a LAN server, so — like its SSH-access override — its identity has
     * nowhere else to live. Null on a Fjord that has not yet been assigned one; it is minted once, on
     * first use, and persisted here.
     */
    private String fjordServerMachineId;

    /**
     * The passphrase that opens this fleet's {@link SurvivalKit}, or null until the operator has chosen one.
     *
     * <p>Fjord holding it looks wrong for a second — it is the one secret the design says the operator keeps
     ...
     * thing the whole feature exists for — Fjord can <em>rewrite</em> the kit when a passphrase changes,
     * where a Fjord that could not would be left nagging about a kit going stale in the operator's absence.
     ...
     *
     * <p>Not a secret and not encrypted — it is a digest of a page whose every line Fjord already stores
    ...
    /**
     * The Fjord server's own identity, or empty when it has not been assigned one yet or the stored value
     * is unusable.
     *
     ...
     */
    public Optional<MachineId> fjordServerIdentity() {
        if (fjordServerMachineId == null || fjordServerMachineId.isBlank()) {
            return Optional.empty();
        }
        try {
            return Optional.of(MachineId.of(fjordServerMachineId));
     ...
     */
    public FjordConfig withSurvivalKitPassphrase(String passphrase) {
    ...
    public FjordConfig withSurvivalKitFingerprint(String fingerprint) {
        return toBuilder()
     ...
     * is persisted here, in this owner-only-readable config file — it is Fjord's own store for the
     * notifier credentials.
     */
    public FjordConfig withSmtpSettings(String newSmtpHost, int newSmtpPort,
    ...
    /**
     * A copy with the Fjord-server SSH-access override replaced (#311); every other field carries over
     * unchanged. A null value clears the override, reverting the effective state to the default (on).
     */
    public FjordConfig withFjordServerSshAccess(Boolean sshAccess) {
        return toBuilder()
            .fjordServerSshAccess(sshAccess)
            .build();
    }

    /** A copy with the host-disk alert threshold replaced; every other field carries over unchanged. */
    public FjordConfig withDiskMonitorThreshold(int thresholdPercent) {
    ...
    public FjordConfig withBackupScheduleHour(int hour) {
