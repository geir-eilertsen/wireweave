...
public record LockoutWarning(List<BlockDecision> decisions) {
     ...
     *                        an alarm on information Fjord could not assemble would be a guess.
     */
    ...
    public String subject() {
        if (decisions.size() == 1) {
            return "[Fjord-O-Matic] Lockout warning: your own " + decisions.get(0).sourceIp()
                + " is blocked at the edge";
        }
        return "[Fjord-O-Matic] Lockout warning: " + decisions.size()
     ...
     * end it. {@code baseDomain} builds the Fjord UI link, omitted when null or blank.
     */
    public String body(String baseDomain) {
        ...
        body.append("\nThis is not an attack. The trusted networks allowlist — your VPN, the Docker bridge, the\n")
            ...
            .append("Something of yours is now being turned away at the edge, and you may lose access to Fjord\n")
