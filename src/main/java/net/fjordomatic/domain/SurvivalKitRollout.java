 ...
 *
 * <p><b>The local copy never counts.</b> The Fjord server gets a kit too, because the likeliest failure by
 * far is a Fjord that will not start on a host whose disk is fine. But it dies with the machine it is meant
 * to outlive, so a rollout that reached no fleet machine has not produced anything that survives Fjord,
...
public final class SurvivalKitRollout {
     ...
     *
     * @param copiesKept fleet machines only — the Fjord server's own copy is never counted
     */
    public record Result(int copiesKept, List<Failure> failures) {

        /**
         * Whether this rollout produced anything that outlives the Fjord server. False when no fleet machine
         ...
         */
        public boolean survivesLossOfFjord() {
            return copiesKept > 0;
        }

        /**
         * Whether every destination took this kit — every chosen machine and the Fjord server's own copy.
         *
    ...
    /** Write {@code kit} to every chosen machine and to the Fjord server, attempting all of them. */
    public static Result distribute(SurvivalKitHosts.Selection selection, String kit,
        ...
        try {
            keeper.keepOnTheFjordServer(kit);
        } catch (RuntimeException e) {
            ...
            failures.add(new Failure("the Fjord server", describe(e)));
        }
