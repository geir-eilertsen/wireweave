 ...
 * good outcome of a bad update, and Fjord says so instead of reporting a generic error.
 *
...
public record ContainerUpdate(MachineId machineId, String containerName, String image,
     ...
     * @throws ConflictException when it has one Fjord will not update, carrying the plain reason
     */
     ...
     * {@link ContainerUpdateEligibility#NOT_COMPOSE_MANAGED}: Fjord holds no compose coordinates for the container,
     ...
     * Fjord's log without the domain doing the logging.
     *
     ...
     * writes its reason to stderr and Fjord captured it and threw it away, so a real failed update said
    ...
    public record Settlement(ContainerUpdateOutcome outcome, String diagnostic) {
         ...
         * though Fjord is quoting a host that stayed silent.
         */
     ...
     *
     * <p><b>Forgotten, never stamped up to date.</b> Fjord pulled and recreated; it did not ask the registry
     ...
     * Claiming up to date would be asserting a verdict Fjord never took.
     *
     ...
     * on the same door, and it covers the container name too, which Fjord does not choose either.
     */
