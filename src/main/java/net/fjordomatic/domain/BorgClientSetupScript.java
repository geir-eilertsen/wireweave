 ...
 * Fjord provisions the borg <em>server</em> but never prepared the client. This closes that gap.
 *
...
public final class BorgClientSetupScript {
    ...
    public static String generate() {
        ...
        sb.append("#\n");
        sb.append("# Fjord Backup client setup — idempotent, safe to re-run. Installs the borg client this\n");
        ...
        sb.append("echo \"==> Fjord Backup client setup complete.\"\n");
        return sb.toString();
    }
     ...
     * only borg</em>, as root without a password. Fjord runs borg as the credential user (e.g. {@code ubuntu}),
     ...
     * Fjord's SSH credential for that machine as powerful as root, and the operator is told so at the toggle.
     *
    ...
    private static String sudoersDropIn() {
        StringBuilder sb = new StringBuilder();
        sb.append("echo \"==> Granting borg-as-root to the Fjord SSH user...\"\n");
     ...
     * {@code SUDO_ABSENT} otherwise. The client install needs root and Fjord SSHes as a non-root user, so
     * this decides whether Fjord can run the install itself (detached under sudo) or must degrade to staging
    ...
    /** Read a {@link #passwordlessSudoProbe} result: {@code SUDO_OK} means Fjord can install borg itself. */
     ...
     * with one client-specific difference: the install needs root and Fjord SSHes non-root, so the script is
