 ...
 * job editor. Containment can say nothing about those — Fjord cannot tell which files a glob bites into — so
...
public record Excludes(List<String> patterns) {
     ...
     * class note: Fjord cannot tell which files {@code *.tmp} bites into, so it can neither claim the folder
