 ...
 * be a no-op. Fjord sits at the VPN hub and is the only node with SSH to every machine, so a Transfer is an
 * SFTP read from the source relayed through Fjord's own JVM to an SFTP write on the destination.
 *
