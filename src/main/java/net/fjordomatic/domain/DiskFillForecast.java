 ...
 * back when Fjord only ever read {@code df -P /} and a machine had exactly one disk to speak of. A machine
...
public record DiskFillForecast(String machineName, String mountPoint, int currentPercent,
    ...
    public String forecastSubject() {
        return "[Fjord-O-Matic] " + machineName + " " + mountPoint + " projected full in ~" + approxRunwayHours()
            + "h";
    }

    /**
     * Body for the early-warning email. {@code baseDomain} builds the Fjord UI link, and when it is null
