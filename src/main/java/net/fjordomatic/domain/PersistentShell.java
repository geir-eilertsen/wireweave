...
/**
 * The single place in Fjord that knows how to open a shell on a machine so the shell <em>outlives the
 * Fjord process</em>. When the operator redeploys the Fjord container (a "hot deploy" run from a web
 * terminal on the Fjord host), the JVM dies, the WebSocket dies, and a bare login shell's PTY dies with
...
public final class PersistentShell {
    ...
    /** The reserved prefix that namespaces Fjord's tmux sessions apart from the operator's own. */
     ...
     * Fjord's own {@value #SESSION_PREFIX} session; the operator's other tmux sessions keep their status bar.
     */
     ...
     * that is the whole point of a persistent shell, and it is what lets a reconnect (or a Fjord redeploy)
