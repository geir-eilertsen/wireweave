 ...
 *
 * <p>The mail says what Fjord saw and stops. It never offers to pull: Fjord is read-only about containers, and
...
public record ImageUpdateRollup(List<ScopedImage> images, Map<String, String> machineNames) {
    ...
    public String subject() {
        if (images.size() == 1) {
            return "[Fjord-O-Matic] Update available: " + images.get(0).label(nameOf(images.get(0)));
        }
        return "[Fjord-O-Matic] Update available: " + images.size() + " images";
    }

    /**
     * Body: every image that went stale and the machine it runs on, then the one thing the operator needs to
     * know — that nothing will happen until they act. {@code baseDomain} builds the Fjord UI link, omitted when
     * null or blank.
     */
    public String body(String baseDomain) {
        ...
        body.append("Fjord does not pull or restart anything — updating is your call.\n");
