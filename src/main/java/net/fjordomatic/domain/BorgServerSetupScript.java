...
/**
 * Generates the single bootstrap shell script a host runs to stand up a Fjord-managed borg
 * {@link BackupServer} from nothing. Fjord never creates containers on remote hosts — like
 ...
 * {@code BORG_UID:BORG_GID} (it sets {@code ENSURE_BACKUP_PERMISSIONS=true}). Fjord later writes
...
public final class BorgServerSetupScript {
     ...
     * {@code id -u}/{@code id -g}), so the ownership always matches the SSH user Fjord authorizes keys as.
    ...
    public static String generate(BackupServer server, String ownerUser) {
        ...
        sb.append("#\n");
        sb.append("# Fjord Backup server setup — idempotent, safe to re-run. Stands up a pinned borg server\n");
        ...
        // exact user Fjord writes authorized_keys as. Never hardcode 1000 — the real NAS owner is uid 1029.
        ...
        // This script runs as root, but Fjord authorizes client keys over SSH *as the owner*, and that
        ...
        // Fjord appends client keys to this file over SSH *as the owner*, not as root. Hand it over
        ...
        // the private ssh_host_*_key files are never touched. Fjord reads host_keys.pub over SSH and installs
        ...
        sb.append("echo\n");
        sb.append("echo \"==> Fjord Backup server setup complete.\"\n");
        sb.append("echo \"    Authorize a client key next — Fjord appends it to ${AUTH_KEYS}.\"\n");
        return sb.toString();
    }
     ...
     * taken when Fjord can SSH the machine but cannot drive its docker socket as a non-root user (the Synology
     * NAS), so provisioning degrades to "run this one command". It never curls anything: the setup endpoint
     * sits behind admin auth, so Fjord stages the bytes itself. Like {@link #detachedLaunch}, the script is
     ...
     * user Fjord authorizes keys as. {@code BORG_SERVE_ADDITIONAL_ARGS} is intentionally left empty — it is
