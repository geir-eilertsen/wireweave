...
/**
 * The Fjord server's own public hostnames, derived from the base domain. Replaces the
 ...
 *
 * <p>All three are covered by the operator's single {@code *.<baseDomain>} A record, so Fjord only
 * ever names them — it never creates them (#331).
 */
public record FjordHostnames(String baseDomain) {

    /** The FQDN the Fjord web UI is served on, e.g. {@code vaier.example.com}. */
    public String fjordServerFqdn() {
