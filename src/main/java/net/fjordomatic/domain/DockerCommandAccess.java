...
/**
 * Whether Fjord's SSH user can drive Docker on a machine — a fact about the <b>machine</b>, not about any
 * container on it.
 *
 * <p><b>Why Fjord cannot infer it from anything it already knows.</b> The container scrape reads Docker's
 * API over the tunnel and needs no Unix group at all, so a machine whose SSH user is not in its
 * {@code docker} group scrapes perfectly and looks entirely healthy — while every command Fjord would run
 ...
 *
 * <p><b>Learned on a trip Fjord already makes.</b> The five-minute disk sweep signs in to every credentialed
 ...
 *
 * <p><b>Held, never persisted</b>, and re-read on every sweep: it is what Fjord last saw, and a machine
 * whose operator has just fixed the group heals on the next pass without anything to reset.
 */
public enum DockerCommandAccess {

    /** The probe ran and Docker answered: Fjord can drive Docker on this machine as its SSH user. */
    GRANTED,
    ...
    /**
     * Nobody has looked yet, or the last trip came back without an answer. <b>Not a no.</b> Fjord holds no
    ...
    /** Whether Fjord knows it cannot drive Docker here. Only a verdict actually taken says yes. */
     ...
     * fact Fjord holds and re-offer an action already known to be doomed. Same shape as
