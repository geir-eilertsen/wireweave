...
/**
 * What survives Fjord: every key to every backup, and the commands that use them.
 *
 * <p><b>Why this has to exist.</b> Everything needed to read Fjord's backups is currently inside Fjord. The
 ...
 * in the store being backed up. Losing the Fjord server therefore leaves an encrypted repository whose
 ...
 * someone with a laptop, no fleet and no Fjord, so they cannot be locked behind anything that was lost with
...
public final class RecoverySheet {
    ...
    public static String render(BackupServer server, List<BackupRepository> repositories,
        ...
        sb.append("work on a day when there is no Fjord left to ask. If you redirected this to a file,\n");
        ...
        if (repositories == null || repositories.isEmpty()) {
            ...
            for (BackupRepository repo : repositories) {
                ...
                row(sb, "Passphrase", repo.passphrase() == null || repo.passphrase().isBlank()
                    ? "NOT STORED — Fjord does not hold this one; find it yourself before you need it"
                    : repo.passphrase());
                sb.append("\n");
            }
        }
        ...
        sb.append("Fjord server, not the key below.\n\n");
        ...
        sb.append("restoring Fjord itself from one of them yields a configuration whose stored credentials\n");
     ...
     * holding on the day there is no Fjord to ask. Normally the job that targets the repository names the
     ...
     * from before Fjord, or one created before repositories were named by identity.
     */
