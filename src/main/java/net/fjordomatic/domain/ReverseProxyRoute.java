...
public class ReverseProxyRoute {
     ...
     * address+port — a container on Fjord's own network that also publishes a host port has two spellings
    ...
    /**
     * True iff this route is one Fjord persists in its own Traefik dynamic-config file and can
     ...
     *       relay peer's tunnel or the Fjord server's own LAN is up; the route can be
    ...
    public State hostState(List<DockerService> localServices, List<VpnClient> vpnClients,
        ...
        if (isLanService) {
            ...
            // Routability through Fjord still wins over reachability — a known-up LAN host with
            // a dead relay tunnel is unreachable to operators outside the LAN, and reporting
            // UNKNOWN there would hide a real outage.
            boolean routable = anchor.isFjordServer() || anchor.relayPeer()
     ...
     * Fjord-server container, matched by container name (the usual persisted address) or, failing
     ...
     * has no matching container deliberately does <em>not</em> fall back to Fjord-server
     * matching, so an unrelated local container on the same port is never mis-attributed.
     *
     * @param fjordServerContainers        containers on the Fjord server itself
    ...
    public Optional<DockerService> backingContainer(
        ...
        return fjordServerContainers.stream()
            ...
            .findFirst()
            .or(() -> firstPublishedOnPort(fjordServerContainers));
    }
     ...
     * is the un-gated way in: everything Fjord itself fetches from a published service (its icon,
     * say) reaches it here rather than through {@code https://<fqdn>}, which for a social-gated
     * route answers 401 to any request that carries no oauth2 cookie — and Fjord carries none.
     */
     ...
     * is routed through a relay peer; {@code FJORD_SERVER} runs on this Fjord instance itself;
    ...
    public ServiceLocation serviceLocation(List<VpnClient> vpnClients,
        ...
        return resolveServer(vpnClients, peerIdResolver, peers).id() == null
            ? ServiceLocation.FJORD_SERVER
            : ServiceLocation.PEER_SERVER;
    }

    public enum ServiceLocation { FJORD_SERVER, PEER_SERVER, LAN_SERVICE }
    ...
    private static String stripRedundantPeerSuffix(String subdomain, ServerIdentity server) {
        if (server.id() == null) return subdomain; // Fjord server / unresolved — no peer to strip
     ...
     * peer's name for a LAN service, or {@link LanAnchor#FJORD_SERVER_NAME}. The launchpad groups
     * and labels tiles by this (issue #209): VPN peer names are the operator-set display labels,
     * the Fjord server name is fixed.
     */
     ...
     * peer whose tunnel address it is, or the Fjord server for anything else (its own hub routes, and a
    ...
    public Optional<MachineId> hostMachineId(List<PeerConfiguration> peers, List<LanServer> lanServers,
        ...
        return peers.stream()
            ...
            .or(() -> Optional.ofNullable(fjordServerId));
    }
    ...
    /**
     * The peer hosting this route, as an {@code id} (the immutable slug — null for the Fjord
    ...
    private ServerIdentity resolveServer(List<VpnClient> vpnClients, ForResolvingPeerIds peerIdResolver,
        ...
        if (isLanService) {
            ...
            return relay != null
                ...
                : ServerIdentity.FJORD_SERVER;
        }
        // Check VPN peers first — a peer IP is unambiguous, whereas port-only Fjord-server
        // matching can produce false positives when a Fjord-server container happens to use the same port.
        boolean isPeer = vpnClients.stream().anyMatch(p -> p.containsAddress(address));
        if (!isPeer) {
            return ServerIdentity.FJORD_SERVER;
        }
    ...
    /** A route's host machine: the peer {@code id} (null for the Fjord server) and its display name. */
    private record ServerIdentity(String id, String displayName) {
        static final ServerIdentity FJORD_SERVER = new ServerIdentity(null, LanAnchor.FJORD_SERVER_NAME);
    }
