...
/**
 * The catalogue of which containers running on the Fjord server may be offered as publishable
 * services. The containers of Fjord's own stack are hidden; the operator's own containers on the
 * same host are offered on every TCP port they expose. A few of Fjord's own are a deliberate
 * carve-out — offered, but only on the ports that are worth publishing, and with a default
 * root-redirect path.
 *
 * <p>The names below are the {@code container_name}s in Fjord's {@code docker-compose.yml}, spelled
 ...
 * to the auth chain's naming. {@code FjordServerCatalogueTest} reads the compose file and fails the
 ...
 */
public final class FjordServerCatalogue {

    private FjordServerCatalogue() {}

    /** Containers of Fjord's own stack that are never offered as publishable services. */
    private static final Set<String> EXCLUDED = Set.of(
        // Fjord, the tunnel, and the tunnel's masquerade sidecar.
        ...
        "docker-proxy",
        // The "Fjord is down" placeholder, and the access-log rotator.
    ...
    /**
     * The carve-out: containers Fjord knows specifically and offers only on the listed ports. Traefik's
     * dashboard is the one part of Fjord's own stack an operator has a reason to publish — behind auth,
    ...
    /** Whether {@code containerName} is one of Fjord's own infrastructure containers. */
    public static boolean isExcluded(String containerName) {
        return EXCLUDED.contains(containerName.toLowerCase());
    }

    /**
     * Whether {@code containerName} is a container of Fjord's own stack that Fjord deliberately offers.
    ...
    /**
     * Whether a container running <b>on the Fjord server</b> is one of Fjord's own — hidden or offered
     * alike. Offered is not the opposite of Fjord's own: Traefik is offered for publishing and is still
     * part of the stack a Fjord release pins. Says nothing about a container of the same name on a peer
     * or a LAN server, which is the operator's.
     */
    public static boolean isFjordOwnStack(String containerName) {
        return isExcluded(containerName) || isOffered(containerName);
    }

    /**
     * The Fjord-server containers an <b>update sweep</b> may ask registries about: the operator's own, and
     * nothing of Fjord's own stack (#353).
     *
     * <p><b>Why they are dropped rather than swept and hidden.</b> These images are pinned by a Fjord
     * release and move with it, so the only honest resolution of a mark on one is "wait for a Fjord
     ...
     * reading a verdict from here, so the mark buys nothing — and on a host that builds Fjord locally the
     ...
     *
     * <p><b>Only meaningful for the Fjord server.</b> A peer or LAN server running its own {@code traefik}
     ...
     */
    public static List<DockerService> sweepable(List<DockerService> fjordServerContainers) {
        if (fjordServerContainers == null) {
            return List.of();
        }
        return fjordServerContainers.stream()
            .filter(container -> !isFjordOwnStack(container.containerName()))
            .toList();
    }
