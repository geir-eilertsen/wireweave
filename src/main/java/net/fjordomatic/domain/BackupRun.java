...
public record BackupRun(
     ...
     * client simply was never installed, and Fjord can install it (see {@code PrepareBackupClientUseCase}).
     *
    ...
    public static BackupRun borgMissing(BackupJob job, String runId, Instant now, String machineLabel) {
        return failed(job, runId, now, BORG_MISSING + machineLabel
            ...
            + "“Get this machine ready”, and Fjord will install it.");
    }
    ...
    public String failureSubject(String machineLabel) {
        ...
        return "[Fjord-O-Matic] " + what + jobName + " on " + machineLabel;
    }
    ...
    public String recoverySubject(String machineLabel) {
        return "[Fjord-O-Matic] Backup recovered: " + jobName + " on " + machineLabel;
    }

    /**
     * Body for the admin failure / recovery email — job, machine, repository, exit code, finish instant
     * and the {@code summary} tail (never the passphrase), ending with a link to the Fjord UI built from
