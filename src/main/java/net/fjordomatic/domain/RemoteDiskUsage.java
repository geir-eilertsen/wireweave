 ...
 * for any machine in the fleet, including the Fjord host reached via SSH-to-self. It owns the business
 ...
 * Fjord and could have filled to 100% without a word. So a reading is now a <em>list</em> of filesystems,
...
public record RemoteDiskUsage(String machineName, String device, String mountPoint,
     ...
     *
     * <p>This is a <b>domain decision</b>, not a parsing detail: it decides what Fjord is willing to call a
    ...
    private static RemoteDiskUsage parseRow(String machineName, String line) {
        ...
        try {
            ...
            // A block count too large for a long — absurd, but a reading Fjord cannot trust is no reading.
            return null;
        }
    }
     ...
     * and hands back the whole answer: is Fjord watching, what is it judging against, and is this disk in
     * trouble.
     *
    ...
    /**
     * What Fjord has decided about one filesystem: whether it is watched, the threshold it was actually judged
     * against (its own, or the global fallback), and whether it is breaching.
     *
     * @param watched         whether Fjord alerts on this filesystem at all
    ...
    public record DiskVerdict(boolean watched, int thresholdPercent, boolean breaching) {
        ...
        /**
         * Whether Fjord says <b>nothing at all</b> about this filesystem — no level alert, and no fill
         * forecast either.
         *
    ...
    public String pressureSubject() {
        return "[Fjord-O-Matic] " + machineName + " " + mountPoint + " is at " + usedPercent + "% full ("
    ...
    public String recoverySubject() {
        return "[Fjord-O-Matic] " + machineName + " " + mountPoint + " is back to " + usedPercent + "% full ("
            + availableHuman() + " free)";
    }
     ...
     * and {@code baseDomain} builds the Fjord UI link, omitted when it is null or blank.
     */
