...
/**
 * The file that outlives the Fjord server: {@link RecoverySheet}'s contents, encrypted with one passphrase
 * only the operator knows, written to hosts Fjord picks for separation.
 *
 ...
 * because a kit that needed Fjord to read it would restore the circularity the kit exists to break. The
 * instructions are worth nothing without the passphrase, and everything with it.
 *
 * <p><b>What it has to survive.</b> Only the Fjord server. The archives themselves exist in exactly one place
 ...
 * is written for is "Fjord is gone, the NAS is fine".
 */
public final class SurvivalKit {
    ...
    public static String render(BackupServer server, List<BackupRepository> repositories,
        ...
        sb.append("==================\n\n");
        sb.append("Everything needed to read this fleet's backups without Fjord: where the archives are, and\n");
        sb.append("the passphrases that open them. It is encrypted with a passphrase only you know — Fjord\n");
        ...
        // passphrase, on a machine that no longer has a Fjord to ask how old its kit is.
        sb.append("Written ").append(WRITTEN_AT.format(writtenAt)).append(" by Fjord, covering ")
