...
/**
 * How often the operator may make Fjord go and ask the registries (#57 slice 3).
 *
 ...
 * remembered answer — so it is the one path in Fjord where a human can turn impatience into a hundred manifest
 ...
 * deferred: the operator gets an immediate, true answer — when Fjord last really looked — instead of a spinner
...
public class UpdateCheckFloor {
     ...
     * @param lastCheckedAt when Fjord last really looked: now when admitted, the earlier check when refused.
