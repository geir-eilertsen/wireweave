...
/**
 * The single place in Fjord that knows the borg command line. Given a {@link BackupJob} and its
...
public final class BorgCommand {
     ...
     * Fjord never has to re-implement that invariant.
     *
     ...
     * reconciled with reality after a Fjord restart: the in-memory registry does not survive a restart, so a
     * {@code borg mount} left live on a host would otherwise never be swept. Asking the host what is mounted
     * lets Fjord adopt and reap those orphans (which hold the repository lock and block backups). A missing
     ...
     * for any one compromised client to read and delete every other host's repositories — so Fjord never
     ...
     * {@link BorgServerSetupScript} and {@link BackupServer#hostKeysPath()}) so Fjord can pin them on clients.
     ...
     * and never has to trust-on-first-use. Fjord is the trusted broker: it obtained these keys over its own
     ...
     * directory exactly once. Fjord is the thing that moved it, so it already knows the answer. Needed only
    ...
    private static String buildCreateBody(BackupServer server, BackupJob job, BackupRepository repo, String borg,
        ...
        // Keep borg's own state and Fjord's work dir out of every archive, so a run whose source paths
