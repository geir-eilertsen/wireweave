...
/**
 * A driven port the domain calls to lock a {@link net.fjordomatic.domain.SurvivalKit}'s contents with the one
 ...
 * keyed by a file on the Fjord server, which is precisely the machine a survival kit assumes is gone. A kit
 ...
 * a kit has lost their fleet; they cannot be asked to install anything, and Fjord is not there to be asked.
 */
public interface ForEncryptingSurvivalKits {
     ...
     * the ciphertext on several machines, so the realistic attack is not against Fjord at all — it is someone
