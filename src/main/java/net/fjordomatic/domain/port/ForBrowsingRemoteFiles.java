 ...
 * Fjord is the only node with SSH to every machine, so it is the only place a fleet-wide file tree can
 * be assembled.
 *
...
public interface ForBrowsingRemoteFiles {
     ...
     * {@code .} to. One half of the pair of answers {@link net.fjordomatic.domain.SftpRoot} is resolved from: an
     ...
     * half comes from {@link net.fjordomatic.domain.port.ForRunningSshCommands}, not from here.
     */
     ...
     *
     * <p><b>The order is the domain's</b> ({@link net.fjordomatic.domain.SftpRoot#jailCandidates}), and so is the
     ...
     * is already there — the byte sink for an {@link net.fjordomatic.domain.Upload}, piped with a fixed buffer so
     ...
     * sessions are held open concurrently for the duration and both are closed before returning — Fjord's
     ...
     * ordinary state of a fleet ({@link net.fjordomatic.domain.NotFoundException},
     * {@link net.fjordomatic.domain.PermissionDeniedException}) — it is <b>skipped</b>, never allowed to abort a walk
