...
/**
 * The Fjord server's own {@link MachineId} — the one machine identity that cannot be found by searching,
 * because the Fjord server is neither a VPN peer nor a LAN server and appears in no machine store. It lives
 * in the Fjord config, and this port is the only way to ask for it.
 *
 ...
 * the Fjord server before any assigning caller had run got no identity at all, and the machine it was trying
 ...
 *
 * <p>Assigning is deliberately confined here and happens <b>once</b>. Everywhere else in Fjord identity is
 * <em>read, never minted</em>: a stored id that is missing or malformed must not quietly become a different
 * machine, because everything keyed to the old one would be orphaned in silence. The Fjord server is the one
 ...
 */
public interface ForResolvingFjordServerIdentity {

    /**
     * The Fjord server's identity, assigning and persisting one if it has never had it. Never null: a caller
