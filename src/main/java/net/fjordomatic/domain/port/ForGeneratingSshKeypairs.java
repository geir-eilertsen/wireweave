...
/**
 * Driven port for the SSH key-material Fjord mints itself (#309): generating a keypair for a machine it
 ...
 * they share one port rather than earning two. Generation happens in Fjord's own process, unlike
 * {@code BorgCommand.ensureClientKeyPair}, which shells {@code ssh-keygen} on a host Fjord can already
...
public interface ForGeneratingSshKeypairs {
     ...
     * passphrase — the vault already encrypts it at rest, and a passphrase Fjord holds beside the key it
     ...
     * operator pastes into a host's {@code authorized_keys}. Works for any key Fjord can read, so a
