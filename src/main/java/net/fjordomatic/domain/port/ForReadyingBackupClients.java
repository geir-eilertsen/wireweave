 ...
 * the decision the {@link net.fjordomatic.domain.BackupJob} makes on a machine's FIRST back-up: trust the machine's
...
public interface ForReadyingBackupClients {
     ...
     * is allowed to opt in to {@link net.fjordomatic.domain.BackupJob#backupAsRoot()}: a job that asks for root on a
     ...
     * (detached); {@code scriptOnly} when Fjord could not run it itself and the operator must run the staged
