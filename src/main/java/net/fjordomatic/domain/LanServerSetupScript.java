 ...
 * Fjord. It adapts to what the host needs:
 * <ul>
 *   <li>when {@code dockerPort != null} — installs Docker (if absent) and exposes the engine API
 *       on {@code tcp://0.0.0.0:<port>} so Fjord can scrape it (native + Snap installs);</li>
...
public final class LanServerSetupScript {
     ...
     *   <li>installs routes via the relay peer when the server is anchored at one (not at the Fjord
    ...
    public static Optional<String> forHost(LanServer server, List<PeerConfiguration> allPeers,
        ...
        if (anchor.isPresent() && !anchor.get().isFjordServer()) {
     ...
     * reach the Fjord server's subnet, VPN peers, and other sites' LANs). The host's own relay LAN is
    ...
    public static String generate(String machineName, String expectedAddress, Integer dockerPort,
        ...
        // Lock the (unauthenticated) Docker API only when we know the source Fjord's scrape arrives from —
        // the relay gateway. A docker host with no relay (on the Fjord server's own LAN) keeps the printed
        ...
        sb.append("#\n");
        sb.append("# Fjord LAN host setup — idempotent, safe to re-run. This host needs:\n");
        if (doDocker) sb.append("#   - Docker engine API exposed on tcp://0.0.0.0:").append(dockerPort).append("\n");
        if (doRoutes) sb.append("#   - static routes to the Fjord server subnet (and other site LANs) via its relay peer\n");
        sb.append("#\n");
        ...
        sb.append("\necho\n");
        sb.append("echo \"==> Fjord LAN host setup complete.\"\n");
        if (doFirewall) {
            sb.append("echo \"    Docker API (tcp ").append(dockerPort)
                .append(") is locked to the Fjord fleet gateway ").append(gateway)
            ...
            sb.append("echo \"    Docker API is on tcp://0.0.0.0:").append(dockerPort)
                ...
                .append(" from the Fjord server only (it is unauthenticated; never expose it to the internet).\"\n");
        }
        return sb.toString();
    }

    /**
     * Locks the (unauthenticated) Docker API to the fleet. Fjord's scrape reaches a relay-anchored host
    ...
    private static String firewallBlock(int port, String gateway) {
        String fw =
            "#!/bin/sh\n"
            + "# Managed by Fjord — locks the unauthenticated Docker API to the fleet gateway. Reconciling and\n"
        ...
        return "\n"
            + "echo \"==> Locking the Docker API (tcp " + port + ") to the Fjord fleet gateway " + gateway + "\"\n"
            ...
            + "[Unit]\n"
            + "Description=Fjord — lock the Docker API to the fleet gateway\n"
            ...
            + "else\n"
            + "    echo \"    WARNING: iptables not found — could not lock the Docker API; restrict tcp " + port + " to the Fjord fleet by hand.\" >&2\n"
            + "fi\n";
    }
    ...
    private static String dockerBlock(int port) {
        return "\nPORT=" + port + "\n" + """
            ...
            echo "    Reminder: allow inbound TCP ${PORT} from the Fjord server in this host's security group / firewall."
            """;
    }
    ...
    private static String routeBlock(String gateway, List<String> cidrs) {
        ...
        sb.append("# NOTE: routes to other sites' LANs only carry traffic once Fjord issue #250 ships\n");
        ...
        sb.append("[Unit]\n");
        sb.append("Description=Fjord LAN static routes via relay peer\n");
