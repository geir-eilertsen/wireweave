...
/**
 * Fjord could not read a machine's disk: the {@code df} did not run (a sleeping machine, a non-zero exit,
 * a timeout) or its output could not be parsed.
 *
 * <p>This exists so that "cannot tell" is never rendered as a number. A disk Fjord failed to read is not a
 * disk with room on it, and reporting {@code 0%} — or falling through to the generic 500 "An unexpected
 * error occurred" — would be Fjord claiming to know something it does not. It carries the machine's name so
...
public class DiskUnreadableException extends RuntimeException {
    ...
    public DiskUnreadableException(String machineName) {
        super("Fjord could not read the disk on \"" + machineName + "\". The machine may be asleep, or its "
