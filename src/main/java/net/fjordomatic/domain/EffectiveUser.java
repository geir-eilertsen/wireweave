...
/**
 * The user Fjord acts as on a machine — the login name in that machine's {@link HostCredential} — and
 * whether that user is privileged. It is the machine's blast radius, said out loud: every file Fjord
 ...
 * Fjord's delete there can remove something the machine needs to boot; an Ubuntu server logs in as an
 ...
 *
 * <p><b>What the judgement is, and what it is not.</b> Fjord judges by the login name it connects with,
 * because that name is what fixes the blast radius of everything Fjord does over SFTP on that machine.
 ...
 * non-root user with passwordless sudo is still <b>unprivileged</b> for Fjord's own file operations,
...
public record EffectiveUser(String username, boolean privileged) {
    ...
    /** The one login name Fjord treats as privileged. */
    private static final String ROOT = "root";

    /**
     * The effective user for a login name, deciding privilege from the name itself — or {@code null}
     * when there is no login name at all, which is what "Fjord holds no credential here" looks like.
     * Surrounding whitespace is never part of a login name and is trimmed; case is significant, because
     * on Linux {@code Root} is a different account and Fjord must not claim it is uid 0.
     */
