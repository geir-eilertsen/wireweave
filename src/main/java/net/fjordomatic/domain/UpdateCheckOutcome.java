 ...
 *
 * <p>The pairing exists so the UI cannot round either one up. "Fjord checked and found nothing new" and "Fjord
 ...
 * @param lastCheckedAt when Fjord last really looked: now for an admitted check, earlier for a coalesced one
 */
public record UpdateCheckOutcome(boolean checked, boolean changed, Instant lastCheckedAt) {
    ...
    /**
     * A check the floor refused. Fjord did not ask, so it does not get to say it asked — it reports when it
