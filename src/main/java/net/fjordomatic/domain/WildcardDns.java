...
/**
 * The single {@code *.<domain>} A record that is the whole DNS story for a Fjord install without AWS
 * credentials. Every service Fjord publishes lands under it, so there is exactly one thing to get
 ...
 * Fjord publishes machine-qualified hostnames two labels deep, e.g.
 ...
 * Probing {@code <random>.<random>.example.com} exercises the depth Fjord actually publishes at.
 */
