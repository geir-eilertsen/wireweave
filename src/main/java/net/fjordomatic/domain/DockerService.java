...
/**
 * One container as Fjord sees it on a host, plus what Fjord has decided about its image.
 *
 ...
 * @param updateAvailable Fjord's verdict on this container's image. {@link UpdateAvailability#UNKNOWN} on a
 ...
 *                        it carries no compose labels Fjord will act on — a plain {@code docker run}.
 * @param updateEligibility whether Fjord may offer to update this container's image, and why not when it
...
public record DockerService(
    ...
    /**
     * This container carrying Fjord's verdict on whether its image may be updated. Everything the host
     ...
     *
     * <p>Docker Compose names a network {@code <project>_<name>}, so the network Fjord's own stack
    ...
    /**
     * How {@code port} of this container is reached from the Fjord server: on the Fjord network,
     ...
     */
    public Optional<ServiceEndpoint> reachableEndpoint(PortMapping port, String fjordNetworkName,
                                                       String dockerGatewayIp) {
        if (isOnNetwork(fjordNetworkName)) {
     ...
     * with. A container that is both on the Fjord network and publishing a host port has two spellings of
     * the same service, and only asking about the preferred one makes an already-published service look
     * unpublished: routes written before Fjord could see the container on its own network hold the gateway
     * spelling, and would go on being offered as candidates forever.
     */
    public List<ServiceEndpoint> everyReachableEndpoint(PortMapping port, String fjordNetworkName,
                                                        String dockerGatewayIp) {
