...
import net.fjordomatic.adapter.driven.SshConnector.Connection;
...
public class MinaSftpAdapter implements ForBrowsingRemoteFiles {
     ...
     * it with the exec channel's {@code $HOME} is what reveals the jail ({@link net.fjordomatic.domain.SftpRoot}).
     *
     ...
     * ({@link net.fjordomatic.domain.SftpRoot#jailCandidates}), and what a hit <em>means</em> is the domain's too.
     *
     ...
     * asked here: the domain ({@link net.fjordomatic.domain.Upload}) has already decided that before a byte is sent.
     ...
     * may not read are <b>ordinary states of a fleet</b>, not faults — Fjord's SSH users are not root on most
     * machines, so much of a filesystem is legitimately unreadable. Collapsing them into a transport failure
     * would surface as a 500 and tell the operator that Fjord broke, when in truth the answer is simply "that
