 ...
 * this is the one place in Fjord that speaks {@code cscli}, and splitting the two directions across two
 * classes would only duplicate the container name and the exec idiom.
 *
 * <p>This class is the only place in Fjord that knows what a CrowdSec failure looks like — a dead exec, an
...
public class CrowdSecCliAdapter implements ForDetectingIntrusions, ForLiftingBlocks {
     ...
     * than {@code []} — valid JSON, simply not an array — and an alert Fjord cannot parse is skipped
     * per-entry. Everything else is Fjord failing to ask: a dead exec, or output that is not JSON at all,
    ...
    public List<BlockDecision> getActiveDecisionsOrFail() {
        ...
        if (!alerts.isArray()) {
            throw new BlockDecisionsUnreadableException(
                "Fjord could not read who CrowdSec is blocking: cscli answered with something other than a "
                    + "list of alerts.");
        }
    ...
    private JsonNode readAlerts() {
        try {
            ...
            throw new BlockDecisionsUnreadableException(
                "Fjord could not ask CrowdSec who it is blocking: " + e.getMessage(), e);
        }
    }

    /**
     * {@code cscli decisions delete -i <ip>} — the one command Fjord ever issues that changes CrowdSec's
    ...
    public void liftBlock(SourceAddress address) {
        try {
            ...
            throw new BlockNotLiftedException(
                "Fjord could not lift the block on " + address.value() + ": " + e.getMessage(), e);
        }
    }
