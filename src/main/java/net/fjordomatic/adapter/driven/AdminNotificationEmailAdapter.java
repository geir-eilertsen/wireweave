...
import net.fjordomatic.domain.PendingIdentity;
import net.fjordomatic.domain.FjordConfig;
import net.fjordomatic.domain.port.ForNotifyingAdmins;
import net.fjordomatic.domain.port.ForPersistingAccessEntries;
...
import net.fjordomatic.domain.port.ForSendingNotificationEmail;
...
/**
 * Driven adapter that turns a composed alert (subject + body) into an email to every Fjord admin,
 * and raises the new-pending-identity alert. Both {@link ForSendingAdminNotification} (the shared
 * recipient-resolve+send primitive, used by {@code NotificationService} for every scheduler alert)
 * and {@link ForNotifyingAdmins} (the forward-auth access-request alert, used by the user domain)
 * used to live on {@code NotificationService} — a {@code *Service} must not implement a driven
 * ({@code For*}) port, so the SMTP machinery moved here where it belongs.
 */
@Component
@Slf4j
public class AdminNotificationEmailAdapter implements ForSendingAdminNotification, ForNotifyingAdmins {
    ...
    public AdminNotificationEmailAdapter(ForPersistingAccessEntries accessStore,
    ...
    public void sendToAdmins(String subject, String body, String context) {
        Optional<FjordConfig> maybeConfig = configPersistence.load();
        ...
        FjordConfig config = maybeConfig.get();
