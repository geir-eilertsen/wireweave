...
public interface PrepareBackupClientUseCase {
     ...
     * {@link #prepareClientStatus}. The install needs root and Fjord SSHes as a non-root user, so it first
     * probes for passwordless sudo: with it, Fjord runs the script itself under {@code sudo -n}; without it
     ...
     * when Fjord could not run it itself (no passwordless sudo, or the host is not reachable) and the operator
     * must run the staged script; {@code message} is a human-readable reason for any path (never a secret).
     *
     * <p>{@code stagedScriptPath} is the absolute on-host path Fjord wrote the install script to when it could
