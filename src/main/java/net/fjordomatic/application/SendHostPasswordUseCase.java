...
/**
 * Send a machine's stored SSH password straight from Fjord into an open terminal session, so the
 * password travels from the vault to the SSH server without ever reaching the browser. It is written
 * only when the remote is actually awaiting a password (see {@link net.fjordomatic.domain.PasswordPrompt}) —
