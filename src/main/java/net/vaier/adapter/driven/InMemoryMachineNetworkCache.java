 ...
 *
 * <p>Ephemeral on purpose. A reading is a fact about a network that can change under Vaier's feet, so
 * losing it on a restart is correct — the next sweep re-reads it within five minutes, and until then
 * Vaier simply says nothing rather than offering a stale network. Nothing here is state an operator
