...
/**
 * A driven port the domain calls to put a {@link net.vaier.domain.SurvivalKit} somewhere it will still be
 * found later — on a machine in the fleet, or on the Vaier server itself.
 *
 ...
 * day Vaier cannot be asked anything. Where on the host it lands, and how it gets there, is the adapter's
...
public interface ForKeepingSurvivalKits {
    ...
    /**
     * Write the kit on the Vaier server itself. Never one of the fleet copies — a kit that dies with Vaier is
     * not redundancy — but the copy that is there on the far likelier day when Vaier will not start and its
     * disk is fine.
     */
    void keepOnTheVaierServer(String content);
}
