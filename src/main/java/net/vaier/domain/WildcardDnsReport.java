 ...
 * check is that "did I set DNS up right?" stops being a question and becomes a sentence Vaier states,
 ...
 *                          — two labels deep, because that is the depth Vaier publishes at (see
 *                          {@link WildcardDns})
...
public record WildcardDnsReport(
    ...
    /** A sentence an operator can act on without knowing anything about how Vaier works. */
    public String message() {
        return switch (status) {
            ...
            case UNCONFIRMED -> wildcardName() + " resolves to " + observed()
                + ", but Vaier could not determine this server's own public address, so it cannot "
