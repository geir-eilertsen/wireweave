...
Slices 2 (presence from the app on the browser's rules) and 3 (rotate and revoke) are unbuilt. The app itself now exists and Vaier hands it out — see the second addendum below.
...
The **launchpad**'s topbar gains an **Android** item for every viewer, signed in or not — the only nav item that never depends on a session — painted only after a `HEAD` on the endpoint answers 200, so Vaier never offers what it cannot serve. The Explorer's phone handoff now names the app first, with a link to the launchpad, and keeps the QR underneath as the fallback for the WireGuard app. CI builds the APK on the runner before the image, signing from three repository secrets and falling back to the debug key when they are absent, so a fork without them still produces a working image.
