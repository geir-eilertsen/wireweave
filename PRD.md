...
- [x] **C — Machines ✅.** A machine's **containers**, **published services** and **disk** are **entries**, so
  ...
  **Containers were read-only, deliberately.** At the time of this slice, `DockerServiceRestController` was
  ...
  holds — start, stop, restart and logs remain unshipped). **One verb has since shipped: Upgrade ([#352](https://github.com/getvaier/vaier/issues/352), §6.37).** It is `POST`, not `GET`, and it is the one exception to "containers are read-only" — everything else this paragraph says still stands.
...
- **Container control endpoints** — start / stop / restart a container, and read its logs. Vaier still has
  ...
  **shell** is where that is done. (**Upgrade**, [#352](https://github.com/getvaier/vaier/issues/352), §6.37,
...
**What shipped.** Everything behind an **upgrade** except the button. Vaier could already tell an operator an image was out of date and could offer them nothing to do about it; it can now carry the upgrade out on any container in the fleet the endpoint is asked about — the **Vaier server**, a server **VPN peer**, a Docker-enabled **LAN server** all judge eligibility and would honour the request. In practice the Explorer only ever offers the button where the update-available mark can appear today, which is the Vaier server and server peers — LAN-server containers still read `UNKNOWN` (the §6.8 backlog gap), so a LAN-server container has nothing to offer an Upgrade action on yet, even though the backend is already ready for the day that gap closes.

**The container carries its own address.** Every scrape now reads the four compose labels off each container into **compose coordinates** (`domain.ComposeCoordinates`), and every scrape stamps an **upgrade eligibility** verdict on each container, judged against *which machine it came from* — the same container name means Vaier's own stack on the Vaier server and the operator's container anywhere else, which no name can say by itself.

**The upgrade is a domain object** (`domain.ContainerUpgrade`). It decides whether the upgrade may happen at all, builds the two compose commands, owns their deadlines, and reads each result. Two runs, never one: `pull` and `up -d` are issued separately so that **a failed recreate is its own outcome** — the old container is still running, which is the good ending of a bad upgrade, and Vaier says so rather than reporting a generic error. Every `-f` file the project was defined by appears, in order; a project recreated from its first file alone drops what its override files said. The five **upgrade outcomes** carry the operator's sentence with them, so the browser is handed words rather than a code to invent words from.
