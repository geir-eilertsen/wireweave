...
2. User creates `.env` with `VAIER_DOMAIN`, `ACME_EMAIL`, the Google OAuth credentials (`VAIER_OIDC_GOOGLE_CLIENT_ID` / `VAIER_OIDC_GOOGLE_CLIENT_SECRET`), `VAIER_ADMIN_EMAIL`, and `VAIER_PUBLIC_HOST` / `VAIER_PUBLIC_IP` when not on EC2
