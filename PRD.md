# Product Requirements Document — Fjord-O-Matic
...
Fjord is a self-hosted infrastructure management tool for developers running a homelab. It eliminates the manual work of maintaining a WireGuard VPN server and a reverse proxy by providing a single interface that wires everything together automatically. DNS is one **wildcard DNS** record the operator creates once, before first boot (§6.4).

The core value proposition: add a new Docker service anywhere on your VPN, select a subdomain, and Fjord handles the reverse proxy and HTTPS — end to end.

Fjord is a personal tool that will be open-sourced. It is not intended to compete with general-purpose infrastructure platforms (Portainer, Coolify, Rancher, etc.). It is opinionated about its stack: WireGuard + Traefik + Google sign-in (via oauth2-proxy). It is deliberately **not** opinionated about DNS — any host that can serve one wildcard `A` record qualifies.

### 1.1 The name ✅

The product was **Vaier** until August 2026 and is now **Fjord-O-Matic**, short form **Fjord**. Implemented in one change: the Java package `net.vaier` → `net.fjordomatic`, the Maven coordinates, every `Vaier*` type, all operator-facing copy, the four living docs, and the wordmark — whose highlighted infix moved from the "ai" of V-**ai**-er to the "**-O-**" of Fjord-O-Matic.

**Deliberately excluded — the stack identifiers.** The container, network, volume paths, `VAIER_*` environment variables, the `getvaier/vaier` image, the `getvaier/vaier` GitHub repository, `vaier-config.yml` and its `vaierServer*` keys, `/var/lib/vaier-backup`, and the `vaier.<domain>` console host all keep their names. This is what makes the rename a no-migration upgrade. Renaming them is a **backlog** item and a real migration: it re-issues Let's Encrypt certificates, moves borg repository paths on every fleet host, and changes the WireGuard `SERVERURL` — which peers cache indefinitely, so it would need a physical power-cycle at each site (see the endpoint-cache failure mode in `docs/NETWORKING.md`).

---

## 1.1 Design Philosophy: Automation First, Zero Config

The defining characteristic of Fjord is that **things should just work**. The user should never need to set an environment variable or edit a config file to enable a feature that Fjord can detect or infer automatically.
...
Each step is done in a different tool, with no feedback loop. Mistakes are silent (wrong IP, missing middleware, typo in DNS name). Fjord removes all of this — and step 2 stops being per-service work entirely, because one **wildcard DNS** record made at install answers for every name Fjord will ever publish (§6.4).
...
- Values automation over flexibility — happy to adopt Fjord's opinionated stack
...
- Not a DNS manager at all — Fjord reads DNS, never writes it. The operator owns one wildcard record; see §6.4
...
- **Out-of-date detection** — `GetVpnPeersUseCase` flags each peer's `configOutOfDate` by comparing its on-disk config against its **rendered config** (`WireGuardPeerConfig.isOutOfDate`, server render inputs resolved once per refresh). The Services-page card shows a **Config out of date** badge and styles the **Reissue config** button as primary. The comparison strips the `# VAIER:` metadata comment from both sides first (`stripFjordMetadata`) — that comment is Fjord-side metadata never installed into the tunnel, and it may carry a `deviceCategory` key that `generate()` omits — so editing a peer's device category, name, or description no longer falsely trips the out-of-date badge. A Reissue threads the stored device-category override through `WireGuardPeerConfig.reissue` so the override survives the re-render.
...
**Current capabilities:**
- Discover containers with exposed ports on the Fjord server and VPN peers
...
1. User sees two lists on the published services page:
   - **Discovered** — containers with exposed TCP ports not yet published, found on the Fjord server and reachable VPN peers
...
- **Services page card layout** ✅ (closes [#234](https://github.com/getvaier/vaier/issues/234), [#235](https://github.com/getvaier/vaier/issues/235)) — the admin published-services list now groups cards under one section per peer, mirroring how the launchpad presents tiles: section heading is the host (`hostName`, "Fjord" for the Fjord server's own services, the relay peer for a LAN route); peers and services sort alphabetically within their group. Each card's bold heading is the operator-facing Display Name (`launchpadAlias` if set, else the route's `shortName`) — pathPrefix no longer concatenates into the heading. The dim sub-line carries (a) the LAN host's display name as `@ <name>` for LAN services — surfaced via `ReverseProxyRoute.lanServerName(List<LanServer>)` and a new nullable `lanServerName` field on `PublishedServiceUco`, resolved by `lanAddress` against `lan-servers.yml` — so an operator reads "DSM @ NAS" even though the section names the relay, and (b) the pathPrefix when set. Discovered and Processing sections stay flat.
...
- Admin pages (`/admin.html`) and all admin APIs remain protected by social login (Google via oauth2-proxy → Fjord `/authz/verify`, which enforces admin)
...
**DNS is one record the operator makes once, and Fjord never writes DNS again.** Before first boot:

| Record | Type | Value |
|--------|------|-------|
| `*.<domain>` | A | the Fjord server's public IP |

That record answers for the three infrastructure hosts (`vaier.<domain>`, `oauth2.<domain>`, `dex.<domain>`) and for every service Fjord will ever publish. Publishing a service is now **one step**: write the Traefik route. Let's Encrypt is untouched — HTTP-01 still issues a certificate per hostname; the wildcard only makes the name *resolve*, and there is no wildcard certificate anywhere in the stack.

**Stronger than the issue asked for: Route53 was removed, not demoted.** #331's acceptance criteria kept a Route53 mode alongside the wildcard. The decision taken instead was to delete AWS DNS outright, so there is exactly one DNS story and no mode to pick. This makes Fjord **provider-agnostic**: any DNS host that can serve one wildcard `A` record qualifies, which is the point — the newcomer's DNS is wherever they bought the domain, and asking them for an IAM key was the single largest onboarding cliff. Deleted with it: `Route53DnsAdapter`, the AWS SDK route53 dependency, `VAIER_AWS_KEY` / `VAIER_AWS_SECRET` (env *and* `vaier-config.yml`), `PUT /settings/aws` and the AWS credentials UI, `ManualDnsAdapter` and `DnsAdapterConfig`, the entire DNS domain (`DnsProvider`, `DnsZone`, `DnsRecord`, `DnsState`, `ForPersistingDnsRecords`, `ForValidatingAwsCredentials`), `DnsService`, `DnsRestController` and its `/dns/*` endpoints, and the Add/Delete DnsRecord/DnsZone + GetDnsInfo use cases. `ForResolvingDns` survives — Fjord still *reads* DNS, it just never writes it.
...
**Boot-time verification (`domain.WildcardDns`).** Since Fjord no longer creates the record, it checks it. At boot `Lifecycle` resolves a random probe name under the domain against a public resolver (`1.1.1.1` / `8.8.8.8` — what Let's Encrypt queries, not the container's split-horizon view) and compares the answer with this server's own public IP (`ForResolvingPublicHost`). The verdict is a `WildcardDnsReport` carrying a `WildcardDnsStatus` — `COVERED`, `NOT_RESOLVING`, `RESOLVES_ELSEWHERE`, `UNCONFIRMED` — and a plain-language sentence naming the one action to take. It is stated in the boot log *and* held in `WildcardDnsStatusHolder` so the Settings pane can state it too (a log line scrolls away; the question "did I set DNS up right?" should be answerable without one). The random labels are minted in `StartupLifecycleRunner`, not in the domain, so the check stays deterministic under test.

**The probe is deliberately two labels deep — do not "simplify" it to one.** Fjord publishes machine-qualified hostnames two labels deep, e.g. `openhab.colina27.eilertsen.family`. A DNS wildcard matches by **closest encloser** (RFC 4592): `*.example.com` stops covering `anything.colina27.example.com` the moment the zone gains *any* real record under `colina27`. A single-label probe (`<random>.example.com`) is answered by the wildcard regardless, so it would report `COVERED` while every machine-qualified service was dead — precisely the failure the check exists to catch. Probing `<random>.<random>.<domain>` exercises the depth Fjord actually publishes at. The two random slices are independent, so neither label can collide with a name the zone really has.
...
Keep the operator aware when a container's image has an **update available**. Reopened after a stale `vaultwarden` image on a server peer silently broke Bitwarden mobile sync: pulling it fixed it in seconds, but nothing in Fjord knew, so nothing could tell the operator. This is that signal.
...
Fjord ships an SMTP notifier that carries Fjord's admin alert emails (machine up/down, disk pressure, new access requests). Settings and the password are stored in `vaier-config.yml` (the password owner-only; read back via `ForReadingStoredSmtpPassword`, implemented by `FjordConfigFileAdapter`). (It previously also powered Authelia's password-reset mail; Authelia has been removed entirely.)
...
- **Server machine up/down alerts** ([#173](https://github.com/getvaier/vaier/issues/173)): two 30s schedulers — one watching WireGuard handshake age for `UBUNTU_SERVER`/`WINDOWS_SERVER` peers, one watching the LAN reachability TCP probe for `LAN_SERVER` machines. Mobile/Windows clients are excluded — their disconnects are routine user behaviour. On a state change either watcher emails every **admin**-role **access entry** with subject `[Fjord-O-Matic] <name> is now <connected|disconnected>` and a body containing the machine's name, type, last handshake (or last-seen timestamp for LAN servers), LAN address, and a link back to `vaier.<domain>/explorer.html` (was `/vpn-peers.html` before the Infrastructure page was deleted). Per-machine state is in-memory; the first observation after Fjord startup is treated as a baseline so a restart never produces a notification storm. No quiet-hours setting — alerts fire 24/7.
- **Reachability debounce for LAN servers**: a probe result must hold for 3 consecutive 30s cycles (≈60s of consistency) before the published cache flips and an email goes out. Dampens both the WireGuard tunnel warmup window after a Fjord restart (no false-down email when it takes one cycle for the relay handshake to complete) and ordinary network flapping (a single transient timeout never propagates). The UI shows the icon as grey ("warming up") until the first state confirms.
...
- **Disk pressure alerts on the first sighting, and escalate by band** ✅ (implemented 2026-07-31) — the Fjord server's own root filesystem reached **89% against an 80% threshold and never sent one email**. Two faults compounded, and neither fix works alone:
...
- **Boot verifies the wildcard record instead of creating anything** (#331, §6.4). Fjord still resolves the server's own public address in order — `VAIER_PUBLIC_HOST` → `VAIER_PUBLIC_IP` → EC2 IMDSv2 `public-hostname` → DNS lookup of `vaier.<domain>` — but now uses it to *judge* the operator's `*.<domain>` record rather than to write records. An unresolvable public address is not fatal: the report settles `UNCONFIRMED` and the stack stays up. *(Superseded: this step used to auto-create `vaier.<domain>` and the two auth-stack CNAMEs.)*
...
- A `@RestControllerAdvice` (`net.fjordomatic.rest.GlobalExceptionHandler`) maps uncaught exceptions to the **API error envelope** — `net.fjordomatic.rest.ApiError(code, message, detail)`, where `code` is a stable machine-readable token, `message` is an operator-safe human-readable explanation, and `detail` is optional/nullable.
- `IllegalArgumentException` — the convention domain validation throughout Fjord already uses to signal bad input — maps to `400` with `code=BAD_REQUEST` and the exception's message surfaced **verbatim**, so operator-readable validation messages reach the UI.
...
### 6.17 Social login + Fjord-owned authorization 🟡 (V2, in progress — Option C from the spike)
...
moves to an external **identity provider** via oauth2-proxy, and **Fjord owns authorization** through a
...
(reaches the services whose **access group** it holds) or **admin** (administers Fjord and reaches every
service). The same store gates **both** the Fjord console (admin-only) and per-service access.
...
- Traefik generation (`TraefikReverseProxyAdapter`): per route, the chain for its mode — `authelia` →
  ...
  `oauth2-authn` Google forward-auth → `vaier-authz` Fjord forward-auth) **plus** a higher-priority
...
- Logout: `FjordHostnames.logoutUrl(AuthMode, target)` is mode-aware — Authelia portal logout vs
...
- Config only (no Java) makes the Dex **connector** id reach Fjord: the provider requests the
...
- No CSP is set anywhere in Fjord (no Spring header, no meta, no Traefik middleware), so no img-src
...
- The avatar resolution chain (GitHub picture → Gravatar `d=404` → placeholder) moved into a shared
  `static/avatar.js` (`FjordAvatar.photoUrl`) — `users.html` was refactored onto it (behaviour
...
- Console auth mode: step 3b added a `VAIER_CONSOLE_AUTH_MODE` env var to select how the Fjord console
  ...
  `FjordHostnames.logoutUrl(AuthMode, …)`, and the docker-compose env line are all gone. Console logout is
...
- Password-change surface removed: the "Change password" card on My Page (`mypage.html`), the per-user
  ...
  endpoint, and `ChangePasswordUseCase` are all gone — social-login users have no Fjord password. The
...
- An expanded VPN-peer card used to show a flat grid mixing editable settings (name, description,
  ...
  Fjord-server cards are unchanged (they carry no WireGuard internals). Static-resource-only change.
...
publishing, or `login.<domain>` CNAME/router creation. `FjordHostnames` lost `autheliaHost()` /
`autheliaLogoutUrl()` / `logoutUrl(AuthMode, …)`, and `mandatoryDnsRecords()` returned only the `vaier` CNAME. *(That method is itself gone now — #331 deleted every record Fjord used to create; see §6.4.)*
...
survive to feed that cleanup and the defensive `FjordServerCatalogue` infra-exclusion. No Authelia runtime, no
Authelia config written, and no dead Authelia Java classes remain.

**Delivered (fresh-install console sign-in fix, TDD-first):** ✅ — the Fjord console's own compose-label
...
- The unauthenticated "awaiting approval" page and migration off Authelia for existing deployments. The
  **availability coupling** the spike flags — Fjord being in the request path for protected services —
...
A per-machine **web terminal**: open an SSH shell to any machine in the Fjord network straight from the
admin UI, with Fjord holding the credentials — enough to replace Termius. Per-host, keyed on a machine
...
- **#311 — Fjord-server host as an SSH target ✅ (credential surface only).** The Fjord server host
  itself now appears in Infrastructure as a singleton synthetic machine (`Machine.fjordServer`, reusing
  `MachineType.UBUNTU_SERVER` + device category `SERVER` so no new `MachineType` and no routing ripple),
  under the reserved name `"Fjord server"` (`LanAnchor.FJORD_SERVER_NAME`, rejected as an operator peer /
  ...
  (`fjordServerSshAccess`, default on); the host credential uses the existing name-keyed vault unchanged.
  `PATCH /machines/{name}/ssh-access` routes the Fjord-server write to the config store; a dedicated
...
- **#308 — Web terminal ✅.** A live in-browser SSH shell per machine: a **Terminal** button on every
  SSH-capable card (peers, LAN servers, the Fjord server) opens a locally-vendored xterm.js
  (`static/vendor/xterm/5.3.0`) over a WebSocket at `/machines/{name}/terminal`. The `TerminalService`
  resolves the machine's **SSH address** (peer tunnel IP / LAN address / Fjord host gateway or
  ...
  `window.parent.fjordOpenTerminal`, which opens a shell as a tab and switches to the Terminal section.
  ...
  binds its height to `visualViewport` so the soft keyboard never covers the prompt. On a phone Fjord also
...
- **Persistent shells survive a Fjord redeploy ✅.** A web-terminal shell used to be a bare login-shell
  PTY living inside the Fjord JVM, so running `docker compose up -d --force-recreate vaier` from a terminal
  on the Fjord host killed the JVM, the WebSocket, and the shell — losing cwd, history, scrollback, and the
...
- **Ending a persistent shell — closing a pane no longer strands a tmux session ✅.** Persistence had no
  ...
  whatever was inside it — 10 orphaned sessions were found on the Fjord server itself, including a `claude`
...
- **Send stored password to a live prompt ✅.** A **Send password** action in each shell's tab actions menu
  asks Fjord to write that machine's stored password straight into the PTY, so it travels vault → SSH server without the
...
- **#313 — Remote SSH exec → host telemetry & alerts** (umbrella). Turn the SSH capability into a
  ...
  - **#314 — Non-interactive SSH exec port ✅ (keystone).** New driven port
    ...
    command can't OOM Fjord. A non-zero exit is a normal result, not an exception; connect/auth/host-key
  ...
  - **#316 — Remote disk-pressure alert** (`df` over SSH) ✅. The first alert built on the exec keystone:
    ...
    **disk watch** and its size, is [#325](https://github.com/getvaier/vaier/issues/325) ✅; see §6.9. It also
    ...
    the combination that let the Fjord host's own root filesystem reach 89% in silence. It now alerts on the
    ...
    local host watcher**: once `RemoteDiskWatcher` could reach the Fjord host itself over SSH-to-self, the
    old `DiskUsageWatcher` (and its `NotifyAdminsOfDiskPressureUseCase` port) were deleted outright to stop
    double-notifying admins for the same host — the Fjord host is now just another `Machine` in the fleet
...
interactive shell plus saved snippets. (Fjord does speak SFTP now, but as its own feature: see **6.20
...
time — and Fjord is the only node with SSH to every machine, so it is the only place a fleet-wide file
...
- **A shared SSH target resolver.** Resolving a machine name to *where to connect, with which credential,
  ...
  *decisions* inside it went to the domain, where they belong: **`SshAddress`** (a peer answers at its
  tunnel IP, a LAN server at its `lanAddress`, the Fjord server host at its resolved host address —
...
- **Two SSH-side browse failures now map to actionable responses instead of a generic `500`** — same "never
  ...
  the credential Fjord holds instead. `ApiError` gains a three-arg `of(code, message, detail)` factory to carry
  the machine name.
...
- **`SshConnectException` and `HostKeyMismatchException` are mapped too ✅ (2026-07-29, #344).** The actual
  ...
  sibling audit of every `net.fjordomatic.domain` exception against the registered handlers found one more of the
...
**Notes / open risks:**
- **`borg mount` is verified ✅** — confirmed end-to-end on the Fjord server (mount + list + read over SFTP with
  Fjord's own credential). The catch the fleet actually hit: Debian's borg runs under a system `python3` shipping
...
- **Browsing is not confined to a subtree by Fjord** — but the *machine* may confine it. There is no chroot
  of Fjord's making: an operator can browse anywhere the machine's SSH user can read, by design (the fleet's
  Docker volumes live outside any one home directory), and the SSH user's own permissions are the boundary.
  Where an SSH daemon chroots its own SFTP subsystem, that jail is real and Fjord maps around it (the **SFTP
...
- **A file has a coordinate; the destination names the operation.** A different machine is a **copy**; the
  browser is a **download**. Fjord sits at the VPN hub and is the only node with SSH to every machine, so a
  cross-machine copy is an SFTP read from A relayed through Fjord's own JVM to an SFTP write on B — no
  host-to-host trust, no keys between peers, nothing new in the network model.
- **The relay runs in Fjord's JVM, not detached on a host.** Unlike `BackupRunner`'s host-detached
  ...
  **Transfer** runs on a background `ExecutorService` and lives in an in-memory registry — an ephemeral live
  ...
  regardless of file size. A Fjord restart simply loses in-flight transfers (acceptable for V1).
...
- **Open a viewable file in the browser ✅.** `GET /machines/{machine}/files/view` (`path`, optional `at`) —
  ...
  relatives — refused outright, because an inline response is served on Fjord's own origin against the
...
- **`UploadFileUseCase` on `ExplorerService`** (a new narrow use case on the existing Explorer domain service —
  ...
  straight to the use case — the bytes are never read whole into Fjord's heap. Admin-authed like the rest of
...
- **The operator makes the decision; the backend absorbs the machinery.** The whole flow an operator sees is
  *select files, click Back up*. Everything under it — the machine's **backup repository**, its **backup job**,
  ...
  per direction. This is the design direction for Fjord generally: shrink what the operator has to hold in
...
- **First back-up readies the host ✅** — a machine's **first** back-up now provisions its **backup client**
  ...
  `POST /machines/{machine}/backup/paths` creates a **new** job for a machine that had none, Fjord (1) trusts the
...
- **"Stop backing up" really stops it — and says so honestly ✅** — selecting a folder *inside* a protected
  ...
  - **Honest end to end**: `DELETE /machines/{machine}/backup/paths` answers `{changed, stopped[], job}` (still
    ...
    *"Nothing changed — Fjord was not backing that up."* when nothing did. A job's Inspector also lists its
    excluded paths under **Not backed up**, beside what it protects.

---

### 6.21 Explorer becomes Fjord's only UI 🟡 (in progress — epic [#323](https://github.com/getvaier/vaier/issues/323))

The **Explorer** (§6.20) stops being *a page for files* and becomes **the tree that is Fjord's operator UI**.
...
service, an **archive**; Fjord is the only machine with SSH to every other, so it is the only place that
...
- **Coverage becomes a property of a path.** A **backup job**'s source paths and a machine's file tree are the
  same namespace, so Fjord can flag an uncovered directory *while the operator is standing in it* — the
...
- [x] **A — The shell ✅.** `explorer.html` is the tree shell: tree rail (fleet root → machines → `files`,
  ...
  moved in unchanged — the shell is the first Fjord page that is not a section inside an iframe.
...
- [x] **B — Files in the tree ✅.** A directory is an **entry**. Expanding one reads it **lazily over SFTP**
  ...
  cached directories with it. Each directory owns its own `FjordListing` reader — and therefore its own
  ...
  **Liveness, corrected.** The rail's status dot knew only WireGuard **peers**, so every **LAN server** — the
  ...
  "Fjord has no idea" when Fjord already knew. The dot now folds in `GET /lan-servers`, whose `status` is a
  **`MachineStatus`** the domain has already decided (`MachineStatus.forLanServer`), so the browser picks a
  ...
  green dot there would be a claim Fjord cannot make. The **Fjord server** is up because it is serving the
...
- [x] **C — Machines ✅.** A machine's **containers**, **published services** and **disk** are **entries**, so
  ...
  **Containers were read-only, deliberately.** At the time of this slice, `DockerServiceRestController` was
  `@GetMapping`s only: Fjord had no endpoint to start, stop or restart a container, and none to fetch logs. So
  no container verb shipped — the Inspector showed image, version, state, ports, networks and container id,
  and offered nothing Fjord could not honour. A control that looks like a verb and does nothing is a lie about
  ...
  **A published service is one thing with three homes** — a container on a machine, a Traefik route, a DNS
  ...
  LAN server, falling back to the relay peer; the hub's own routes on the **Fjord server**; everything else by
  ...
  **Exactly one new endpoint: `GET /machines/{machine}/disk`.** `RemoteDiskWatcher` has taken this reading on a
  schedule since the disk alerts shipped and only ever *emailed* about it — the number Fjord already knew could
  ...
  sentence ("Fjord could not read the disk on X. The machine may be asleep…") rather than falling through to a
  generic 500 — and is **never rendered as 0%**: a disk Fjord failed to read is not a disk with room on it.
...
- [x] **D — Time ✅.** The archive rail (§6.20 slice 3). Scrubbing back re-lights the
  ...
  **Delivered in slice D (frontend, `explorer-shell.{js,css}` + `explorer-listing.js`) — the past is seen, not told:**
  - **The time rail.** A machine's file view grows a horizontal **time rail** — a row of **stops**, one per borg
    **archive** of that machine, laid out newest-nearest-Now, fed by `GET /machines/{machine}/archives`. It appears
    ...
    a cheaper question Fjord has already answered (`jobsOn(machine)` — the job list loads at boot): a backed-up
  ...
  **Delivered earlier in slice D (backend) — the past is a coordinate:**
  - **`borg mount` is the mechanism, and there is only one browse.** A file's third coordinate is *time*, and the
    ...
    present"** and Fjord never re-implements that invariant.
  ...
  - **FUSE verified end-to-end on the Fjord server** (`borg mount ssh://borg@nas::<archive>` then listing/reading over
    SFTP with Fjord's own credential). Apalveien 5 and Colina 27 still need `python3-pyfuse3` — now installed by
    **Prepare client**.
...
- [x] **Repository management in the tree ✅.** A **backup repository** stops being a card you manage on the
  ...
  **New repository** action (name, an optional path override, an append-only toggle, and a strong auto-generated
  ...
  and **Delete**: Delete **forgets the repository in Fjord** (`DELETE /backup-repositories/{name}`) — it does not
...
- [x] **Infrastructure ported, and the page deleted ✅.** The Infrastructure page's whole function moved into the
  ...
  **SSH access** toggle (whether Fjord may open an SSH session, distinct from *storing* the credential — turning
  ...
  **Settings** is native. *(Backups later left the bridge too, when its page was deleted — see "The Backups page
  ...
  link points to `/explorer.html`; and `PeerSnapshot`'s peer-notification "Fjord UI" link points to
...
- [x] **Capability glyphs, the Fjord server's containers, and a fleet-level Add machine ✅.** Four follow-ons
  after the Infrastructure port:
  - The **Fjord server** machine now reports **`runsDocker`** (`Machine.fjordServer`) — the box is itself the
    ...
    `DiscoverFjordServerContainersUseCase` already covered them, which was a quiet gap between the README claim
    and the tree). Its port stays null: Fjord reaches that engine over the local socket, not a TCP port.
  ...
  - **"Repository" leaves the UI ✅.** It is a borg noun the operator never chose: Fjord creates exactly one
    ...
    store and orphaned it, and there is now no normal path that needs it. Adopting a store Fjord did not
  ...
  - **A failed run names a fix that exists ✅.** `BackupRunner`'s borg pre-flight refused a run with "borg is
    ...
    **Get this machine ready**, which POSTs the existing `…/prepare-client` route; where Fjord cannot gain
...
- [x] **Deterministic shell reattach ✅.** Opening a machine's shell no longer **scavenges** a random orphaned
  ...
  machine has one **stable primary** pane id (`FjordPanes.primary`, remembered per browser across reloads and
...
- [x] **One shell model in the Explorer ✅.** The Explorer no longer embeds the bottom terminal dock: its
  ...
  (`FjordPanes.primary`). Closing the window keeps the tmux session alive — it even survives a Fjord restart,
...
- [x] **The Explorer has an address, and the tree stops being the way around ✅.** Five changes that only make
  ...
  - **Where you are is the URL.** The location lived in a JS variable, so a reload dumped the operator back at
    ...
    API: the shell is a static file, so `/explorer/fleet/<id>/files/home` would 404 unless Fjord grew a
  ...
  - **Fjord's own entries left the tree for a topbar Fjord menu.** **Settings**, **Users**, **Security** and
    **Concepts** were a second root in the rail, which made it a forest and made its "Fleet" label a half-truth.
    They are now the **Fjord menu** in the chrome — the one surface that survives the rail being folded away or
  ...
  - **⌘K matched identities, not names ✅ (a live defect).** The palette's index kept only the address path, so
    ...
    **Fjord menu**'s entries, which matters more now they are behind a menu, and reads each row's icon off the
    last path segment rather than `path[1]`, which had given every Fjord entry the fallback file glyph.
  Part of epic [#323](https://github.com/getvaier/vaier/issues/323).
...
- **Container control endpoints** — start / stop / restart a container, and read its logs. Fjord still has
  none of these: the Docker adapters read, and the **docker socket proxy**'s allowlist is what stands between
  Fjord and a fleet-wide remote-exec surface. This is its own change with its own security thinking (who may
...
**Why.** Fjord had no machine registry. It had three — WireGuard config directories, `lan-servers.yml`,
and a flag in the Fjord config — unified only by a read projection that identified machines by their
...
- Identity on all three machine kinds: peer `# VAIER:` metadata, `lan-servers.yml`, `vaier-config.yml`
  (`fjordServerMachineId`, assigned on first use). `Machine.id` is non-null.
...
- **Backup group.** `BackupJob`/`BackupRun`/`BackupServer` hold a `MachineId`; the three file adapters store
  ...
  - **SFTP root cache.** `CachingSftpRootAdapter` remembers a machine's jail under its `MachineId`, taken
    ...
    machine's jail could be served to another that later took its name — silently rewriting every path Fjord
    ...
    said outright that "Fjord's canonical identity for a machine is its name"; it no longer does.
...
- **The SSH path, and then everything else.** `ForResolvingSshTargets.resolve` and
  ...
  on their id, and the Fjord server recognises **itself** by the id in its own config rather than by
  comparing against `LanAnchor.FJORD_SERVER_NAME`. `BackupWorkDirResolver`'s `machineName` parameter died
  ...
  `setMachineSshAccess` searched three stores by name and identified the Fjord server by string
  comparison; it matches on identity in all three now.
- **`ForResolvingFjordServerIdentity`.** The Fjord server's id was read in one layer and minted in
  another, so whether Fjord could reach *itself* over SSH depended on whether something had already loaded
...
**Shell sessions travel by identity too ✅ (2026-07-28) — the last name-keyed thing in Fjord.** A shell is
...
the wrong host.
   ...
   a LAN server on the Fjord server's dot by comparing its relay's *name* to `"Fjord server"`, which a peer
   ...
   **Repositories are named by identity too**, which is a decision with a real cost and was taken
   ...
   Fjord meant to be read by a human with no Fjord — SSH into the NAS and `Apalveien-5` tells you what you
   ...
   **Migration (done on the live instance, by hand).** Three of the four repository directories were moved
   ...
   (Apalveien 5: 10, Fjord server: 10, Colina 27: 7). Moving one turns out to be **three** steps, not one:
   ...
   1. `mv` the directory. borg then refuses the next *non-interactive* access to it — it records where a
      ...
      Verified against the real binary. Every borg command Fjord issues now exports
      `BORG_RELOCATED_REPO_ACCESS_IS_OK=yes`: Fjord is the thing that moves repositories, so the prompt is
   ...
   2. Point Fjord at it (drop the `repoPath` pin, so the path derives from the identity-name again).
   ...
   **Roon server's repository nearly cost a night's backup, and the near-miss was mine.** It failed to move
   with `passphrase … is incorrect`, and the reason was two layers deep. Fjord's *stored* passphrase for it
   ...
   `ensurePassFile` write a **new** file, named for the new repository, from Fjord's wrong stored
   ...
   host into Fjord's store and removing the stale file; the repository holds **6 archives**, so
   ...
   **A repository's passphrase lives in two places and they can drift.** That is the standing hazard this
   exposed: Fjord's store, and a create-if-absent file on each client. Renaming a repository or rotating a
   ...
   **The three orphan directories were deleted** (`NUC-02` 118 MB, `NUC02` 88 KB, `colina27` 45 MB) —
   ...
   failure is upstream and Fjord is relaying borg's own words ("Repository path not allowed",
   "passphrase … is incorrect"), which are worth more than any status Fjord could invent. Found by being
...
2. ~~**Delete the scaffolding**, in one commit with the uniqueness rule.~~ **Wrong shape, found by doing
   ...
   with `window.fjordMachineIdOf`. One name→id lookup survives, named for the moment it serves:
   ...
   **A partial conversion is worse than none, and this slice proved it twice.** Renaming function bodies to
   ...
   2e. **Creating a peer wrote no identity at all — a live defect found by doing 2d.** `fjordJson` never
   ...
   WireGuard server and was then invisible to Fjord**: no machine, no credential, no backup, and no error
   anywhere — `wg show` would list it while `/machines` did not. `WireGuardPeerConfig.generate` now stamps
   the `MachineId` that `VpnService.createPeer` mints, which is the one moment in Fjord an identity is
   ...
   **deleted**. Nothing in the browser resolves a machine by name any more.
   ...
   (plus the Fjord-server-name and address→name maps that existed only to feed it) becomes
   `belongsTo(MachineId)` — **the last `Machine.hasSameName` consumer**. `/machines` marks the Fjord
   server (`fjordServer: true`), retiring six comparisons against the literal string `"Fjord server"`: a
   ...
   **Image-update verdicts were name-scoped, and that was two bugs waiting.** `ScopedImage` keyed
   ...
   now be called "NAS", and the reserved `"Fjord server"` name is reserved no longer. Nothing is keyed to a
...
**Why.** Traefik emitted no security headers at all and set no `tls.options`, so every response — Fjord's
...
referenced from Traefik's *static* config and from Fjord's compose labels, and a reference Traefik cannot
resolve **disables** the router carrying it. Rendering from Fjord — which boots after Traefik — would leave
that window open. It is a second file in the directory; the one Fjord generates is never written to here.
...
- **Security headers** (`X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`)
  ...
  are safe to impose on an application Fjord did not write.
- **Frame guard** (`X-Frame-Options: SAMEORIGIN`) is per-router, on Fjord's own surfaces only — `vaier`,
...
`X-Frame-Options`, a Fjord-shaped router answers with all three, TLS 1.1 is refused while 1.2 and 1.3
...
**Why.** Reading a route back out of Traefik, Fjord decided "this service is authenticated" from the mere
...
untouched. New domain enum `SshServerPresence` (`UNKNOWN` / `PRESENT` / `ABSENT`) — Fjord's *last-known
...
IP, so the Fjord server's own Traefik entrypoint and SSH port are the only genuinely
...
**Bouncer key — a compose-level shared secret, not a Fjord-minted one.** The original design assumed
`crowdsec-bouncer` (`fbonalair/traefik-crowdsec-bouncer:0.5.0`) needed a wrapper entrypoint that
polled for a Fjord-minted key file, exported it, then exec'd the real binary. Hands-on image
...
refreshed on Fjord boot and whenever `VpnService.updateLanCidr`/`deletePeer` runs.
...
avoided once. Collapsed to a single `BlockDecision`, read via the credential Fjord already holds.
...
sweep. Deliberately **not baseline-quiet**: a ban already active the first time Fjord's watcher
...
(`[Fjord-O-Matic] Breach attempt: 24 new block decisions`), actually delivered over the configured SMTP
...
`EXEC=1` socket-proxy path Fjord already has open** (`ForExecutingInContainer`, the same port
...
(implementing both `ForDetectingIntrusions` and `ForLiftingBlocks` — one place in Fjord speaks
...
Fjord never *adds* a ban: CrowdSec's scenarios decide who is blocked. A port called "for blocking
addresses" would have been a lie about the threat model, reading as though Fjord held the block
button. Only one direction of that decision is Fjord's to take, and the port is named for it. Its
...
throws, the operator's decision is not lost to a transient exec failure. **Fjord deliberately does
...
subject (`[Fjord-O-Matic] Lockout warning: your own 10.13.13.6 is blocked at the edge`) and its own body —
...
### 6.30 Fjord says which user it acts as on a machine ✅ (implemented 2026-07-31, closes [#346](https://github.com/getvaier/vaier/issues/346))

**The problem.** Fjord reaches every machine as the SSH user in its **host credential**, and that user's
...
factory `of(username)`; a blank or null login yields `null`, which is what "Fjord holds no credential here"
looks like. It judges by the **login name Fjord connects with**, because that name is what fixes the blast
radius of everything Fjord does over SFTP on that machine. The javadoc is deliberately explicit about what the
...
still unprivileged for Fjord's own file operations, which never go through sudo. That is the honest answer to
...
different Linux account and Fjord must not claim it is uid 0.
...
- The machine **Inspector**'s **SSH access** section states it in a sentence: *"Fjord acts as root on X.
  Everything Fjord does here — reading, writing and deleting — runs unrestricted."* / *"Fjord acts as geir on
...
- The **delete confirmation** gains a sentence on a privileged machine — *"Fjord is root on X, so this can
...
**Backlog.** The obvious next question is the one this deliberately does not answer: *should* Fjord be root
there? A per-machine "act as this user instead" (or a warning when a machine's credential is root and does not
need to be) is a real feature, but it is a change to what Fjord *does*, not to what it *says*, and stating the
fact has to come first. Detecting a uid-0 alias, or probing `sudo -n`, would each cost an SSH round trip for
an answer that does not change Fjord's own file operations — out of scope until something needs it.

---

### 6.31 A refused host key gets its remedy ✅ (implemented 2026-07-31, closes [#345](https://github.com/getvaier/vaier/issues/345))

**The problem.** When a machine's SSH host key changes, Fjord refuses to connect on a **host-key mismatch**
...
one thing only they can make. It also says what happens next — Fjord pins the new key on the next connect — so
...
**Backlog.** Fjord could offer to *verify* the new key rather than only accept it (showing where else on the
...
operator changes their mind about a colleague's office — and Fjord offered no way to see the list, let alone
...
block); `untrustAddress` is one, because **Fjord never blocks an address**. CrowdSec's scenarios decide that,
...
never the mechanism ones: *"your VPN, this server's own container network, and every network Fjord reaches
...
whitelist parser only on container restart, and Fjord deliberately does not restart the engine guarding the
...
*"lasts until you untrust it here"*. Nothing in Fjord's copy calls this decision final any more — including
...
question Fjord could answer itself. It already holds an SSH credential for the machine and already runs `df`
over that exact connection on a five-minute sweep, so the answer was one command away the whole time. Fjord
...
network can change under Fjord's feet, so losing it on restart is correct and the next sweep re-reads it
...
answered this question, and a nudge is a question, not a correction), Fjord actually read a network, and
...
**Fjord server can never be offered its own network** — its detected LAN contains its own uplink address by
...
**Guards, stated as outcomes.** A machine Fjord cannot read is simply not nudged — no guess, no empty form —
...
**Backlog — detection for the Fjord server itself, explicitly out of scope here.** The **server LAN CIDR** is
...
exists for the Fjord server (it is a **machine** like any other and the sweep reaches it via SSH-to-self), so
...
**Backlog — rotating without the gap.** Generating replaces the credential the moment it is minted, so a machine Fjord *can* already reach goes dark until the operator pastes the new public key in by hand. Fjord holds a working login at that instant and could install the key over it — append to `authorized_keys`, verify the new key authenticates, and only then swap the stored credential, keeping the old one if the verification fails. Not built here: it is a different feature (a rotation with a rollback) from "mint a key for a machine you cannot log into yet", and doing it badly locks an operator out of their own host. The manual path stays regardless, since it is the only one that works for a machine with no credential at all.

**Backlog — meeting a `.ppk` halfway.** A PuTTY key is now refused with a message naming it, which is the honest floor; converting one (or an OpenSSH key from a `.pub` sibling) is a step Fjord could take for the operator instead of asking them to run `puttygen`. Deferred: no evidence yet that it happens often enough to justify a key-format converter in the credential path.
...
**What the operator saw.** `traefik:v3.6.14` and `getvaier/vaier:latest` both reading **update available** on the Fjord server — and checking rather than assuming showed the sweep was *right*: upstream re-pushed the pinned tag, as they do for a base-image rebuild. The verdict was true and unactionable. Worse than a mark: `ImageUpdateTracker` is edge-triggered and deliberately not baseline-quiet, so it arrives as **admin mail**, which is exactly what `feedback_notify_only_on_trouble` exists to prevent. An alert whose only honest resolution is *wait for a Fjord release* teaches an operator to filter the channel, and that costs the alerts that matter.

**Fjord's own stack is no longer swept — at all, with no exception.** `FjordServerCatalogue.sweepable` drops it from the containers handed to the sweep, **before any registry is asked**: an image nobody can act on should not spend the rate limit either, and that limit is shared with every image the operator *can* act on. The decision stays in the domain, on the class that already answers *is this Fjord's own stack?* and is build-guarded against `docker-compose.yml` (§6.36), so it cannot drift from what the stack actually runs. It applies **only to the Fjord server**: a peer running its own `traefik` or `redis` is the operator's container and is swept exactly as before — which is why `judgeFjordServerContainers` and `judgeOperatorContainers` are separate, and it has its own test.

**The plan of record kept `vaier` itself; the operator overruled it, and was right.** The reasoning here was that Settings → Update is a real button, so that one mark was actionable. Two things beat it. Settings does not need the mark — it asks the registry about its own image directly — and on a host that **builds Fjord locally** the local digest differs from what Docker Hub serves for `latest`, so the mark reads "newer available" when acting on it would **downgrade**. A mark that talks an operator into a downgrade is worse than no mark. No exception is also a simpler rule than one exception, and the simpler rule is the one that survives.

**Three things that would have made this a regression instead of a fix, all handled and all tested.** *Stale verdicts:* stopping the sweep is not enough on its own, because the last `UPDATE_AVAILABLE` sits in memory and would keep the dot lit until a restart — which is the operator's exact complaint, unfixed. The sweep's write is authoritative (it replaces the verdict map wholesale), so a mark already held goes clear on the very next sweep; there is a test that seeds the pre-#353 verdict and proves it. *The tracker:* it already forgets entries absent from a sweep's verdicts, so an own-stack image that had alerted is un-latched rather than left latched forever — and if such an image ever returned to the sweep stale it would be news again, which is the right answer for a container that stopped being Fjord's own. Both halves are now pinned by tests. *The self-update:* `SelfUpdateRunner` read the sweep's verdict for `getvaier/vaier`, so dropping `vaier` from the sweep would have left **Settings permanently blind to every update there is** — turning this fix into a regression on the one path the operator's decision was justified by. `SelfUpdate.updateAvailable` now takes `ForResolvingRegistryDigest` and asks for itself. Only a genuine digest difference counts: an unreachable or rate-limited registry, a throw, or a locally-built image with no digest all read as *cannot tell*, which is never a reason to recreate the container the fleet's whole control plane runs inside.
...
### 6.44 The map gains a green dot for everywhere Fjord lets someone in ✅ (backend implemented 2026-08-09)

**The mirror image of a threat ping (§6.28).** The Map has shown who the edge keeps *out* since Slice 3, and nothing at all about who gets *in*. The hook was already there and unread: Traefik forward-auths every request to every gated host through `GET /authz/verify`, and the `vaier-authz` middleware has always set `trustForwardHeader: true` — so `X-Forwarded-For` was arriving and being thrown away. On an **ALLOW only**, Fjord now notes where the request came from.

**Aggregated per place, not per address.** An `AccessSource` is a city: its country, its coordinates, how many allowed accesses have come from it, when the first and most recent were, and up to twenty of the people allowed from there. One dot per city, however many addresses a household's ISP cycles through — which is also what keeps the store bounded. Remembered for one month after the most recent access, then pruned. Everything Fjord gates counts one: a **published service**, the console, the **Explorer**, Settings.
...
1. Peer is already connected to VPN (created via Fjord)
2. Developer starts a Docker container on the peer
3. In Fjord → Services, the container appears in the **Discovered** list automatically
...
6. Fjord writes the Traefik route → (optional) social-login middleware chain. There is no DNS step: the name already resolves under the operator's one **wildcard DNS** record (§6.4)
...
2. Fjord generates WireGuard keys, assigns IP from subnet, writes config
3. Developer downloads QR code or docker-compose file
4. Peer is running; developer can see handshake status in Fjord

### 7.3 Learn that a container has an update available

1. Fjord sweeps the fleet's registries once a day and finds an image whose registry now serves a different digest for the tag the container runs
2. Every admin gets one rollup email naming the image(s) that *newly* went out of date — Fjord pulls nothing and restarts nothing
3. Developer updates the container manually on its host

**Success:** the operator hears about a stale image instead of discovering it through the outage it causes. Both halves now exist: the push side (§6.8) is the rollup email, and the pull side is the mark in the **Explorer** — so an operator who opens the tree for any other reason still sees which containers want attention, and a container's Inspector says which of the three verdicts it is (including "Fjord cannot tell"). Fjord pulls nothing either way; step 3 stays the operator's.
...
4. Fjord verifies the wildcard record, states the **wildcard DNS report**, and seeds the **configured administrator** (`VAIER_ADMIN_EMAIL`) as the first admin access entry
...
- **Sub-image versions are pinned** in `docker-compose.yml`; bumps are deliberate, tested, and released with a new Fjord version (no floating `:latest` tags for upstream images)
...
- **Any** DNS provider integration — Cloudflare, Route53, all of them. Fjord reads DNS and never writes it (§6.4)
...
Fjord is "done enough" when:

1. A developer can add a new Docker container on any VPN peer and have it publicly accessible via HTTPS subdomain in under 2 minutes with no manual steps outside Fjord
...
| # | Question | Decision |
|---|----------|----------|
| OQ1 | Should the launchpad be unauthenticated or protected? | Launchpad is public; admin UI is protected by social login (Google via oauth2-proxy → Fjord `/authz/verify`). A dedicated `/launchpad/services` endpoint returns only DNS address and host address (no ports, auth state, or internal details). |
...
**Backup survival kit (next up, designed 2026-07-23).** Everything needed to read Fjord's backups is currently *inside* Fjord, in a circle: repository passphrases are encrypted in its config store; the key that decrypts them (`SecretCipher`, `vault.key`) sits in the same directory; and that directory is backed up to the backup server — encrypted with a passphrase held in the store being backed up. Losing the Fjord server leaves an encrypted repository whose passphrase is inside itself, and every other machine's archives in the same position. Nothing warns, because nothing is broken until everything is.
...
- Decryptable with **standard tools only** — `openssl enc -aes-256-cbc -pbkdf2 -d` — with the exact command in a plaintext header at the top of the file. Needing Fjord to open it would restore the circularity.
- **Fjord selects the hosts**, and says which and why (evidence, like a nudge). The rule: SSH-credentialed, always-on (`deviceCategory`, not a laptop or phone), **not the Fjord server itself**; pick three maximising separation — never two behind the same relay peer. Re-evaluated when the fleet changes; rewritten whenever a repository or passphrase changes.
- **The kit only has to outlive the Fjord server.** The archives exist in exactly one place (the backup server), so if the NAS dies no key helps. The NAS is therefore a *good* host for a copy, not a conflict of interest — the scenario is "Fjord is gone, the NAS is fine".
- What the operator keeps on paper shrinks from a list that rots to **one short passphrase** that does not.

**Amended 2026-07-23: the kit passphrase is stored in the vault.** The design above said "never stored by Fjord", and that was incoherent with the rest of it: a passphrase Fjord does not hold is a kit Fjord cannot rewrite, which reintroduces exactly the staleness that killed the printed sheet — only now Fjord can merely nag about it. Holding it in the credential vault costs nothing, because an attacker who has `vault.key` already has every repository passphrase directly and has no need of the kit. The operator still keeps it in their head, for the day there is no vault to read it from.

**Amended 2026-07-23: a fourth copy on the Fjord server itself.** "Never the Fjord server" argues against the kit living *only* there, and was over-applied into excluding it entirely. The likeliest failure by far is not a lost server but a Fjord that will not start on a host whose disk is fine — and there, a kit in the config directory is one `openssl` command, where the alternative is hand-decrypting `enc:v1:` envelopes out of the config store. It costs no exposure: the copy sits beside `vault.key`, so on that host it is effectively plaintext, but anyone holding `vault.key` already holds every repository passphrase directly. Two conditions: it **never counts as one of the three** (a copy that dies with Fjord is not redundancy, and filling a separation slot with it would look like three copies while being two), and it is **not a host-selection decision** — the Fjord server is the WireGuard server, not a machine in `getAllMachines()`, so this is an unconditional local write in the distribution slice, not a `SurvivalKitHosts` change. The local copy is also the one most likely to go stale (restore the config dir from an old backup and it looks authoritative); the date in the plaintext header is what catches that without the passphrase.

Built so far (2026-07-23): `RecoverySheet` (now plain text, since it is read in a terminal after `openssl`, not in a browser), `SurvivalKit` (header, marker, ciphertext; refuses to write itself without a passphrase), `ForEncryptingSurvivalKits`/`OpensslEnvelopeAdapter` (OpenSSL's `Salted__` envelope reimplemented in Java, so Fjord's container needs no `openssl` binary — with a test that runs the *real* binary on the command the kit prints on its own face), and `SurvivalKitHosts` (the choice of hosts, with a stated reason for every machine chosen and every machine passed over).

**The kit is written and distributed ✅ (2026-07-27).** The passphrase is stored: `FjordConfig.survivalKitPassphrase`, encrypted at rest by the same `SecretCipher` as `smtpPassword`, set through `PUT /settings/survival-kit-passphrase` (`SetSurvivalKitPassphraseUseCase` on `SettingsService`) and refused when blank — an unprotected kit is indistinguishable on its face from a protected one, so the emptiness is rejected where it would be stored rather than where it would be noticed. It is never read back: `GET /settings/config` answers only `hasSurvivalKitPassphrase`. `ForReadingTheConfigKey` (implemented by `SecretCipher`) supplies the **config key** the sheet carries, *read and never minted* — a key generated by the act of writing a kit would decrypt nothing, printed on the one page claiming to decrypt everything. `SurvivalKitRollout` owns what counts as success: every destination is attempted so one sleeping machine cannot cost the fleet its other copies, and the Fjord server's own copy is written every time but **never counted**, because a copy that dies with Fjord is not redundancy. `ForKeepingSurvivalKits`/`SurvivalKitKeeperAdapter` put it there — into the SSH user's `$HOME` (never a path needing root, which would silently stop being written wherever escalation was unavailable) and beside Fjord's own config locally, base64 over the wire because a kit is prose, base64 and shell metacharacters at once and one mangled byte would only be discovered on the day it was needed. Destinations are addressed by `MachineId`, not name (§6.22): a rollout takes minutes across sleeping machines, and a rename inside that window would otherwise resolve to nothing — or to whichever machine had since taken the name. `rest/SurvivalKitWriter` composes the whole thing behind `WriteSurvivalKitUseCase` and `POST /survival-kit`, returning the outcome *and Fjord's reasoning*: the hosts chosen and why, the machines passed over and why, the ones that refused, and `survivesLossOfFjord`. `409`, not a masked `500`, when no passphrase has been chosen — that is a precondition the operator can fix. **The Settings UI ✅ (2026-07-27).** In the Explorer's Settings pane as **Reading your backups without Fjord**, under Nightly backups — the same domain, and the second half of the same promise. Two controls, deliberately two: rolling the passphrase and the write into one Save would push files across the fleet as a side effect of typing a password. The passphrase is **typed, not generated** — the one place Fjord does not mint a secret for you, because a 32-character random string is exactly what nobody carries in their head, and the kit's premise is that the operator carries this one. It is **typed twice**: a mistyped passphrase saves cleanly, encrypts every copy and is indistinguishable from a correct one until the day it is needed. **Write the kit now** is offered only once a passphrase exists (the alternative is a button whose answer is `409`, which is a fact about an endpoint, not an answer to a person). The report is the section's signature: the headline answers the only question a kit is for — *is there now something out there that opens without this server* — in those words rather than as a count of successful requests; each host that holds a copy is shown with Fjord's stated reason under it, in the same two-line shape as a nudge, because it is the same act; hosts that refused take a red edge and never fold away; machines passed over fold behind *Why not the other machines?*, being reasoning rather than trouble. A rollout that reached no fleet machine says so as a failure and says explicitly that the local copy is in place but dies with the machine it is on — two copies presented as three is the lie the printed sheet told. Saving a new passphrase while kits are already out there warns that the copies still open with the old one until they are written again.

**The rewrite triggers ✅ (2026-07-27) — the feature is complete.** A kit rewritten only when someone remembers has the same flaw as the printed sheet it replaced, so Fjord asks itself, on a ten-minute sweep. **Not a dirty flag**: every write path would have to remember to set one, a restart loses it, and it says nothing about kits written before it existed — three ways to believe the fleet is covered when it is not. Instead `SurvivalKitFreshness` fingerprints (SHA-256) *what a kit would say now* and compares it with what was recorded when the fleet was last written; the contents cannot lie about themselves, and one comparison covers causes nobody enumerated — a repository added, a passphrase rotated, a job re-pointed, a machine renamed, a host that left. Of the **sheet**, never the ciphertext: every write mints a fresh salt, so identical contents differ in every byte and would read as a change on every sweep. Two causes the fingerprint cannot see are handled where they happen. A changed **kit passphrase** leaves the words identical while every copy still opens with the old one, so `FjordConfig.withSurvivalKitPassphrase` **forgets the fingerprint** — read downstream as “never written”. A **host that was asleep** makes the rollout partial, and a partial rollout is never *recorded* as written (`SurvivalKitRollout.Result.reachedEveryDestination`), so it returns as a mismatch and is retried — and because that lives on disk rather than in memory, the retry survives a Fjord restart, which an in-memory flag would not. The sweep never throws (a scheduled method that dies takes its schedule with it, and Fjord silently stopping watching is the failure this exists to prevent), and it stays out of the way: no passphrase, nothing written; no repositories, nothing written, because a fleet backing nothing up should not get a file on every host saying so. Verified on the live fleet 2026-07-27: the first sweep after deploy found no recorded fingerprint, wrote Apalveien 5, Colina 27 and the local copy, and recorded what it wrote.
