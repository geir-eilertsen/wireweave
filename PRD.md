...
**Route53 mode ✅ (default when AWS keys present).** Vaier automates DNS through the AWS Route53 API: on first boot it auto-creates the three mandatory infrastructure records the auth stack needs — `vaier.<domain>` (pointing at this server) plus `oauth2.<domain>` and `dex.<domain>` (CNAMEs to the vaier console host, without which oauth2-proxy's startup OIDC discovery against `dex.<domain>` fails and the auth stack crash-loops) — creating only the missing ones and never disturbing an existing record, and a CNAME per published service after that. The mandatory-record set (and that oauth2/dex are CNAMEs to the vaier host) is a domain decision owned by `VaierHostnames`; `Lifecycle` orchestrates ensuring each via the `ForPersistingDnsRecords` port. Backed by `Route53DnsAdapter` and the `ForPersistingDnsRecords` / `ForValidatingAwsCredentials` ports. There is no UI page for general-purpose record CRUD — the REST endpoints exist (`/dns/*`) but the navigation page was never built. Service publishing is the primary path; advanced records are managed in the AWS console.
...
- **Internal-resolution complement (Traefik network aliases).** The public-DNS wait fixes Traefik's own ACME, but service-to-service calls have the sibling problem: oauth2-proxy (and Vaier) run OIDC discovery against `https://dex.<domain>` by its *public* name (the issuer must match, so an internal `http://dex:5556` shortcut is not an option), resolving through the **container** resolver. On a fresh install a too-early lookup gets NXDOMAIN and poisons that resolver's **negative** cache (Route53 SOA negative TTL ~15 min), crash-looping oauth2-proxy on `no such host` long after the record exists. Fix: alias `vaier`/`oauth2`/`dex.<domain>` onto the **Traefik** container on `vaier-network`, so Docker's embedded DNS answers them from its own registry (straight to Traefik over the bridge) before ever forwarding externally — no public DNS, no negative cache. Traefik terminates TLS with the real cert and routes on to the backend. Pinned by a `DockerComposeStructureTest` case.
...
The scanner is surfaced through the Explorer's **Add-a-machine** flow (see **§6.15.1**), on Enterprise instances only — the control is hidden on Community, detected via `GET /license`. The operator picks a machine kind, chooses **A LAN server**, then **picks which LAN to scan** and adopts a discovered host. The flow is **cached-first and SSE-driven, never polling** ([[feedback_frontend_never_polls]]), and each sweep is **targeted to the picked LAN**. This replaced an earlier standalone "Discovered LAN machines" page section and, superseding an interim in-modal "Scan LAN for machines" picker that merely filled the LAN-address field, now registers a discovered host in one **adopt** action rather than pre-filling a form.
