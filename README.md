...
| **Enrolment from the Vaier app** | A phone running the Vaier app joins without meeting the WireGuard app at all. It makes its own key on the phone, opens your browser at Vaier, and you sign in exactly as always; one button adds it. The private half never leaves the phone — Vaier only ever sees the public half — so there is no config to save, no QR to photograph, and nothing to download for that phone ever again. **Get the app from your own Vaier**: the launchpad's **Android** button downloads it, no store and no account, and the button only appears when your Vaier is actually carrying a copy. *(Presence from the app and key rotation are still to come.)* |
